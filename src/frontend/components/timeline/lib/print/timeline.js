...
function getVisGroups() {
    ...
    if (!groups || groups.length === 0) return visGroups;
...
function parseTimelineForPdfPrinting() {
    ...
    if (Object.keys(visGroups).length !== 0) parseVisGroups(visGroups);
}
...
const setupTimeline = function (container, options_in) {
    ...
    for (const attrname in options_in) options[attrname] = options_in[attrname];

    if (container.data("min")) options.start = container.data("min");
    if (container.data("max")) options.end = container.data("max");

    if (container.data("width")) options.width = container.data("width");
    if (container.data("height")) options.width = container.data("height");
    ...
    if (groups.length > 0) tl.setGroups(groups);
    ...
    $("#tl_group")
        .on("change", function () {
            const fixedvals = $(this).find(":selected").data("fixedvals");
            if (fixedvals) {
...
$(document).on('ready', function () {
    ...
    if (printModal.length) {
        ...
        fitToPageField.on('change', function () {
            if ($(this).is(":checked")) {
