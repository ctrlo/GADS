...
class DataTableComponent extends Component {
  ...
  initTable() {
    ...
    $(this.el).on('childRow.dt', (_i, _, row) => {
  ...
  renderFile(data) {
    ...
    data.values.forEach((file) => {
      ...
      if (file.mimetype.match('^image/')) {
        strHTML += `<img alt="Image Preview" class="autosize" src="/file/${file.id}" />`
      } else {
  ...
  getConf(overrides = undefined) {
    ...
    if (conf.serverSide) {
      conf.columns.forEach((column) => {
        column.render = (_, type, row, meta) => this.renderData(type, row, meta)
      })
    }
  ...
  */
  toggleFullScreenMode(buttonElement) {
    ...
    let $full = $("#full-screen-btn");
    $full.removeClass(this.isFullScreen ? 'btn-toggle-off' : 'btn-toggle');
    $full.addClass(this.isFullScreen ? 'btn-toggle' : 'btn-toggle-off');
  }
