    ...
    <div className="content-block__main">
      ...
      >
        {state.widgets.map((widget: WidgetData) => (
          <div key={widget.config.i}
               className={`ld-widget-container ${state.readOnly ? "" : "ld-widget-container--editable"}`}>
            <div dangerouslySetInnerHTML={{__html: widget.html}}></div>
            {!state.readOnly &&
              <a className="ld-edit-button" onClick={state.onEditClick(widget.config.i)}><span>Edit</span></a>}
