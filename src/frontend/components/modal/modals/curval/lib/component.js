...
class CurvalModalComponent extends ModalComponent {
  ...
  setupModal() {
    this.el.on('show.bs.modal', (ev) => { 
      ...
      // The hidden value may contain edited record data, only if the curval
      // has been created or edited in this screen.
      // Convert it into FormData so that it can be submitted using fetch()
      // - there doesn't seem any way to create a FormData object directly from
      // application/x-www-form-urlencoded data. A possible improvement might
      // be to save the hidden value as JSON instead
