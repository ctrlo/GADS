...
const initValidationOnField = (field) => {
  ...
  if (field.hasClass("input--required")) {
    ...
    if (field.hasClass("input--document")) {
      ...
      $fileButton.on("blur", (ev) => {
        validateCheckboxGroup($checkboxGroup);
      });
      $inputEl.on("change", (ev) => {
        validateCheckboxGroup($checkboxGroup);
      });
    ...
    $jsTreeAnchors.on("focus", (ev) => {
      clearTimeout(anchorBlurTimer);
    });
  } else if (field.hasClass("checkboxes--required")) {
    const $checkBoxes = field.find('input[type="checkbox"]');
    $checkBoxes.on("blur change", (ev) => {
      setTimeout(() => {
        validateCheckboxGroup(field);
...
const validateRequiredFields = (form) => {
  ...
  form
    .find(".input--required:not(.input--document), .textarea--required, .select--required")
    .each((i, field) => {
  ...
  form.find(".select-widget--required").each((i, field) => {
    ...
    } else if (!validateCheckboxGroup($(field))) {
      expandCardValidate(field);
      isValidForm = false;
    }
  });
  ...
  form.find(".fieldset--required").each((i, field) => {
    if (!validateRequiredFieldsetCheckboxes($(field))) {
      ...
      if (collapse && collapse.collapse) collapse.collapse("show");
      isValidForm = false;
    }
  });
...
const isHiddenDependentField = (field) => {
  return (
    field.closest(".form-group[data-has-dependency='1'][style*='display: none']").length > 0
  );
};
...
const validateTree = (field) => {
  ...
  if (
    (!$inputEl.length || $inputEl.val() === "") &&
    !isHiddenDependentField(field)
  ) {
    $inputEl.attr("aria-invalid", true);
    addErrorMessage(field, strFieldName, strID);
    field.addClass("invalid");
