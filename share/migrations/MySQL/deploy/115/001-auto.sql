...
-- Created on Fri Sep 11 12:18:01 2026
--
;
...
CREATE TABLE `calc_unique` (
  ...
  UNIQUE `calc_unique_ux_layout_datetime` (`layout_id`, `value_datetime`),
...
CREATE TABLE `calcval` (
  ...
  INDEX `calcval_idx_value_datetime` (`value_datetime`),
  PRIMARY KEY (`id`),
...
CREATE TABLE `current` (
  ...
  INDEX `current_idx_current_version_id` (`current_version_id`),
  ...
  CONSTRAINT `current_fk_current_version_id` FOREIGN KEY (`current_version_id`) REFERENCES `record` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
CREATE TABLE `graph` (
  ...
  INDEX `graph_idx_y_axis_link` (`y_axis_link`),
  PRIMARY KEY (`id`),
  ...
  CONSTRAINT `graph_fk_y_axis_link` FOREIGN KEY (`y_axis_link`) REFERENCES `layout` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB;
--
-- Table: `graph_color`
--
...
CREATE TABLE `user` (
  ...
  INDEX `user_idx_created_by_id` (`created_by_id`),
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `user_fk_created_by_id` FOREIGN KEY (`created_by_id`) REFERENCES `user` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
CREATE TABLE `view` (
  ...
  `is_limit_override` smallint NOT NULL DEFAULT 0,
  `filter` text NULL,
  `instance_id` integer NULL,
  `created` datetime NULL,
  `createdby` bigint NULL,
