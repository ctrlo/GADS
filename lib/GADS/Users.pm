...
{   my $self = shift;
    $self->user_rs->search_rs(
        {},
        {
            columns => [
                'me.id',           'me.surname',
                'me.firstname',    'title.name',
                'me.email',        'organisation.name',
                'department.name', 'team.name',
                'me.created',      'me.freetext1',
                'me.freetext2',    'me.lastlogin',
                'me.value',
            ],
            join     => [ 'organisation', 'department', 'team', 'title', ],
            order_by => 'surname',
            collapse => 1,
        },
    );
...
{   my $self = shift;
    my @users =
        $self->schema->resultset('User')->search({ account_request => 1 })->all;
    \@users;
}
...
{   my ($self, $params) = @_;
    $self->schema->resultset('Organisation')
...
{   my ($self, $params) = @_;
    ...
    $user->discard_changes
        ;    # Ensure that relations such as department() are resolved
...
"A new account request has been received from the following person:\n\n";
    ...
    $text .= $site->register_freetext1_name . ": $new{freetext1}, "
        if $new{freetext1};
    $text .= $site->register_freetext2_name . ": $new{freetext2}, "
        if $new{freetext2};
    $text .=
    ...
    $text .=
        $site->register_department_name . ": " . $user->department->name . ", "
        if $user && $user->department;
    $text .= $site->register_team_name . ": " . $user->team->name . ", "
        if $user && $user->team;
    $text .= "\n\n";
...
{   my ($self, $user) = @_;
    ...
    else
    {
        ...
        else
        {
            ...
            # memory.
            try { $self->_produce_csv($user, $export) } hide => 'ALL',
...
{   my ($self, $user, $export) = @_;
    ...
    my $instances =
        GADS::Instances->new(schema => $self->schema, user => $user);
    ...
    my $guard = $self->schema
        ->txn_scope_guard;    # Try to ensure consistent data between queries

    push @select_columns,
        +{
            max => $self->schema->resultset('User')->correlate('audits')
            ->search({ instance_id => $_->instance_id })
            ->get_column('datetime')->max_rs->as_query,
    ...
    while ($page)
    {
        my @ids = map $_->get_column('id_max'),
            $self->user_rs->search(
                {},
                {
    ...
    my %user_groups =
        map { $_->id => [ $_->user_groups ] }
        $self->user_rs->search({}, { prefetch => { user_groups => 'group' } })
        ->all;

    my %user_permissions =
        map { $_->id => [ $_->user_permissions ] }
        $self->user_rs->search({},
            { prefetch => { user_permissions => 'permission' } })->all;
