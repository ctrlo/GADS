...
has x_axis => (
    ...
    builder => sub {
        $_[0]->_graph && $_[0]->_graph->x_axis && $_[0]->_graph->x_axis->id;
    },
);
...
has x_axis_link => (
    ...
    builder => sub {
        $_[0]->_graph
            && $_[0]->_graph->x_axis_link
            && $_[0]->_graph->x_axis_link->id;
    },
);
...
has x_axis_name => (
    ...
    lazy    => 1,
    builder =>
        sub { $_[0]->x_axis ? $_[0]->layout->column($_[0]->x_axis)->name : "" },
);
...
has y_axis => (
    ...
    lazy    => 1,
    builder => sub {
        $_[0]->_graph && $_[0]->_graph->y_axis && $_[0]->_graph->y_axis->id;
    },
);
...
has metric_group_id => (
    ...
    builder =>
        sub { $_[0]->_graph && $_[0]->_graph->get_column('metric_group') },
);
...
{   my $self = shift;
    ...
    $schema->resultset('Widget')->search({ graph_id => $self->id })
        ->update({ graph_id => undef });
...
{   my $self = shift;
    ...
    $newgraph->{y_axis_stack} = $self->y_axis_stack
        or error __ "A valid value is required for Y-axis stacking";
    !defined $self->y_axis_stack
        || $self->y_axis_stack eq 'count'
        || $self->y_axis_stack eq 'sum'
        or error __x "{yas} is an invalid value for Y-axis",
        yas => $self->y_axis_stack;
    ...
    !$newgraph->{trend}
        || $newgraph->{trend} eq 'aggregate'
        || $newgraph->{trend} eq 'individual'
        or error __x "Invalid trend value: {trend}",
        trend => $newgraph->{trend};
    ...
    !defined $self->x_axis_grouping
        || grep { $self->x_axis_grouping eq $_ } qw/day month year/
        or error __x "{xas} is an invalid value for X-axis grouping",
        xas => $self->x_axis_grouping;

    my $x_axis_col = $self->layout->column($self->x_axis);
    error __ "Dates and grouping must be entered for a custom range"
        if $x_axis_col
        && $x_axis_col->return_type =~ /date/
        && $newgraph->{x_axis_range}
        && $newgraph->{x_axis_range} eq 'custom'
        && (   !$newgraph->{from}
            || !$newgraph->{to}
            || !$newgraph->{x_axis_grouping});

    error __
"A single field must be selected for the x-axis when using the historic trend option"
        if !$newgraph->{x_axis} && $self->trend;

    $newgraph->{group_by} = $self->group_by;
    !defined $self->group_by
        || $self->layout->column_this_instance($self->group_by)
        or error __x "Invalid group by value {group_by}",
        group_by => $self->group_by;

    $newgraph->{metric_group} = $self->metric_group_id;
    !defined $self->metric_group_id || $self->metric_group_id =~ /^[0-9]+$/
        or error __x "Invalid metric group ID format {id}",
        id => $self->metric_group_id;
    !defined $self->metric_group_id
        || $self->schema->resultset('MetricGroup')->find($self->metric_group_id)
    ...
    $newgraph->{group_by} && $newgraph->{trend}
        and error __
        "Historical trends cannot be used with y-axis grouping (Group by)";
    ...
    error __x
"For graph {title}, a field returning a numeric value must be used for the Y-axis when calculating the sum of values",
        title => $self->title
        if $self->y_axis_stack eq 'sum'
        && !$self->layout->column($self->y_axis)->numeric;
...
{   my ($self, $values, %options) = @_;
    ...
    notice __x "Updating y_axis_stack from {old} to {new} for graph {name}",
        old  => $self->y_axis_stack,
        new  => $values->{y_axis_stack},
        name => $self->title
        if $options{report_only}
        && $self->y_axis_stack ne $values->{y_axis_stack};
    $self->y_axis_stack($values->{y_axis_stack});
    notice __x "Updating y_axis_label from {old} to {new} for graph {name}",
        old  => $self->y_axis_label,
        new  => $values->{y_axis_label},
        name => $self->title
        if $options{report_only}
        && $self->y_axis_label ne $values->{y_axis_label};
    ...
    notice __x "Updating x_axis_link from {old} to {new} for graph {name}",
        old  => $self->x_axis_link,
        new  => $values->{x_axis_link},
        name => $self->title
        if $options{report_only}
        && $self->x_axis_link != $values->{x_axis_link};
    $self->x_axis_link($values->{x_axis_link});
    notice __x "Updating x_axis_grouping from {old} to {new} for graph {name}",
        old  => $self->x_axis_grouping,
        new  => $values->{x_axis_grouping},
        name => $self->title
        if $options{report_only}
        && $self->x_axis_grouping ne $values->{x_axis_grouping};
    ...
    notice __x "Updating stackseries from {old} to {new} for graph {name}",
        old  => $self->stackseries,
        new  => $values->{stackseries},
        name => $self->title
        if $options{report_only}
    ...
    notice __x "Updating trend from {old} to {new} for graph {name}",
        ...
        name => $self->title
        if $options{report_only}
        && ($self->trend || 0) != ($values->{trend} || 0);
    $self->trend($values->{trend});
    my $from = $values->{from}
        && DateTime::Format::ISO8601->parse_datetime($values->{from});
    ...
    $self->from($from);
    my $to = $values->{to}
        && DateTime::Format::ISO8601->parse_datetime($values->{to});
    ...
    notice __x "Updating x_axis_range from {old} to {new} for graph {name}",
        old  => $self->x_axis_range,
        new  => $values->{x_axis_range},
        name => $self->title
        if $options{report_only}
        && ($self->x_axis_range || '') ne ($values->{x_axis_range} || '');
    $self->x_axis_range($values->{x_axis_range});
    notice __x "Updating is_shared from {old} to {new} for graph {name}",
        old  => $self->is_shared,
        new  => $values->{is_shared},
        name => $self->title
    ...
    notice __x "Updating user_id from {old} to {new} for graph {name}",
        ...
        name => $self->title
        if $options{report_only}
        && ($self->user_id || 0) != ($values->{user_id} || 0);
    ...
    notice __x "Updating metric_group_id from {old} to {new} for graph {name}",
        old  => $self->metric_group_id,
        new  => $values->{metric_group_id},
        name => $self->title
        if $options{report_only}
