...
has '+no_value_to_write' => (default => 1,);

has '+value_field' => (default => 'id',);
...
has refers_from_value_field => (is => 'lazy',);
...
{   my ($self, $record_ids, %options) = @_;
    ...
    if (   @all_current_ids == 1
        && $self->layout->cached_records_autocur->{ $all_current_ids[0] })
    {
        my $record =
            $self->layout->cached_records_autocur->{ $all_current_ids[0] };
    ...
    else
    {
        ...
        my $records = GADS::Records->new(
            ...
            columns      => $self->curval_field_ids_retrieve(
                %options, already_seen => $child
            ),
            limit_current_ids => \@all_current_ids,
            include_children  => 1
            , # Ensure all autocur records are shown even if they have the same text content
        ...
        while (my $record = $records->single)
        {
            ...
            $self->layout->cached_records_autocur->{ $record->current_id } =
                $record;
        }
    }
