...
{   my $self = shift;
    if (!$self->userinput)
    {
        ...
        # child column
        return 1
            if $self->schema->resultset('LayoutDepend')->search(
                {
                    layout_id             => $self->id,
...
has display_fields => (
    ...
    coerce  => sub {
        ...
        return GADS::Filter->new(as_json => $val,);
    },
    builder => sub {
        my $self = shift;
        my @rules;
...
{   my $self = shift;
    if (my @display = $self->schema->resultset('DisplayField')
        ->search({ layout_id => $self->id })->all)
    {
        my $conds = join '; ', map {
            $_->display_field->name . " " . $_->operator . " " . $_->regex
        } @display;
        my $type =
            $self->display_condition eq 'AND'
...
{   my $self = shift;
    ...
    {
        $perms{ $p->group_id } ||= [];
        push @{ $perms{ $p->group_id } },
            GADS::Type::Permission->new(short => $p->permission);
    }
    \%perms;
}
...
{   my $self = shift;
    ...
    foreach my $perm ($self->schema->resultset('LayoutGroup')
        ->search({ layout_id => $self->id })->all)
    {
    ...
    {
        $return .= qq(Group "$group" has permissions: )
            . join(', ', @{ $groups{$group} }) . "\n";
    }
...
{   my ($self, $original) = @_;
    ...
    if ($self->type eq 'curval')
    {
        ...
        $self->multivalue(1)
            if $self->show_add && $self->value_selector eq 'noshow';
    }
...
{   my ($self, $record_ids) = @_;
    ...
    {
        my ($left, $prefetch) =
            %{ $self->tjoin };    # Prefetch table is 2nd part of join
...
{   my $self = shift;
    ...
    if (
        my @deps = $self->schema->resultset('DisplayField')->search({
    ...
    {
        my @depsn =
            map { $self->layout->column($_)->name } @{ $self->depended_by };
        my $dep = join ', ', @depsn;
        error __x
            "The following fields contain this field in their formula: {dep}.
            Please remove these before deletion.", dep => $dep;
    }

    # See if any autocur fields depend on this
    if (
        my @autocurs = $self->schema->resultset('Layout')->search({
        ...
        )
    {
        ...
        my $p  = join ', ', @pn;
        error __x
            "The following fields in another table refer to this field: {p}.
    ...
    if (
        my @parents = $self->schema->resultset('CurvalField')->search({
            child_id => $self->id
        })->all
        )
    {
        my @pn =
            map { $_->parent->name . " (" . $_->parent->instance->name . ")" }
            @parents;
        my $p = join ', ', @pn;
        error __x
            "The following fields in another table refer to this field: {p}.
    ...
    if (
        my @linked = $self->schema->resultset('Layout')->search({
            link_parent => $self->id
        })->all
        )
    {
        ...
        my $l  = join ', ', @ln;
        error __x
"The following fields in another table are linked to this field: {l}.
        ...
        )
    {
        ...
        error __x
"The following graphs references this field: {graph}. Please update them before deletion.",
            graph => $g;
    }
    ...
    if ($rs_live->count)
    {
        ...
        error __x
"The following reports reference this field: \"{report}\". Please update them before deletion.",
            report => $r;
    }
    ...
    foreach my $filter (
        $self->schema->resultset('Filter')->search({
            layout_id => $self->id,
        })->all
        )
    {
    ...
    $self->schema->resultset('ViewLayout')->search({ layout_id => $self->id })
        ->delete;
    $self->schema->resultset('ViewGroup')->search({ layout_id => $self->id })
        ->delete;
    $self->schema->resultset('Filter')->search({ layout_id => $self->id })
        ->delete;
    $self->schema->resultset('AlertCache')->search({ layout_id => $self->id })
        ->delete;
    $self->schema->resultset('AlertSend')->search({ layout_id => $self->id })
        ->delete;
    $self->schema->resultset('Sort')->search({ layout_id => $self->id })
        ->delete;
    $self->schema->resultset('Sort')->search({ parent_id => $self->id })
        ->delete;
    $self->schema->resultset('LayoutDepend')
        ->search({ layout_id => $self->id })->delete;
    $self->schema->resultset('LayoutGroup')->search({ layout_id => $self->id })
        ->delete;
    $self->schema->resultset('DisplayField')
        ->search({ layout_id => $self->id })->delete;
    $self->schema->resultset('AlertColumn')->search({ layout_id => $self->id })
        ->delete;

    $self->schema->resultset('Instance')
        ->search({ sort_layout_id => $self->id })
        ->update({ sort_layout_id => undef });
...
{   my ($self, %options) = @_;
    ...
    if ($newitem->{name_short} && !$self->internal)
    {
        ...
        $self->name_short =~ /^[a-z][_0-9a-z]*$/i
            or error __
"Short names must begin with a letter and can only contain letters, numbers and underscores";
        ...
        my $exists = $self->schema->resultset('Layout')
            ->search($search, { join => 'instance' })->next;
        $exists
            and error __x
"Short name {short} must be unique but already exists for field \"{name}\"",
    ...
    {
        my $link_parent =
            $self->schema->resultset('Layout')->find($self->link_parent_id);
        ...
        if ($link_parent->type eq 'curval')
        {
            ...
            {
                error __x
qq(Cannot link to column "{col}" which contains columns from this table),
    ...
    !$newitem->{aggregate} || $newitem->{aggregate} =~ /^(sum|recalc)$/
        or error __x "Invalid aggregate value {agg}",
        agg => $newitem->{aggregate};
    ...
    if ($newitem->{aggregate} && $newitem->{aggregate} eq 'recalc')
    {
      # For a recalc, all dependent fields will need to have a aggregate defined
        ...
        {
            error __x
"In order to recalculate this field from other aggregate values, all "
                . "columns in its calculation need to have a defined aggregate. Currently "
                . "column {name} does not have an aggregate defined.",
                name => $self->layout->column($depend)->name
    ...
    elsif (!$newitem->{aggregate} && $self->id)
    {
        ...
        my @d = grep $_->layout->aggregate && $_->layout->aggregate eq 'recalc',
            @depends;
        my $d = join ', ', map $_->layout->name, @d;
        error __x
"The aggregate on this column cannot be removed, as the following columns "
    ...
    {
        $newitem->{group_display} = $self->group_display
            && $self->group_display eq 'unique' ? 'unique' : undef;
    }
    ...
    unless ($options{report_only})
    {
        ...
        if (!$self->id)
        {
            ...
            $newitem->{position} =
                ($self->schema->resultset('Layout')->get_column('position')
                    ->max || 0) + 1
        ...
        my %write_options = $self->write_special(
            rset     => $rset,
            id       => $new_id || $self->id,
            old_rset => $old_rset,
            %options
...
{   my ($self, $permission) = @_;
    ...
    return 0
        if !$self->userinput
        && $permission ne 'read';    # Can't write to code fields
    ...
    if ($permission eq 'write')      # shortcut
    {
        return 1
            ...
            || $self->layout->current_user_can_column($self->id,
                'write_existing');
    }
    0;
}
...
{   my ($self, %options) = @_;
    ...
    # set)
    my $search = { layout_id => $id, };

    $search->{group_id} = { '!=' => [ '-and', @groups ] }
        if @groups;

    my @removed = $self->schema->resultset('LayoutGroup')->search(
        $search,
        {
            select => {
                max => 'group_id',
    ...
    foreach my $group_id (@groups)
    {
        ...
        {
            push @new_permissions, 'write_existing_no_approval'
                if grep $_ eq 'write_existing', @new_permissions;
            push @new_permissions, 'write_new_no_approval'
                if grep $_ eq 'write_new', @new_permissions;
        }
        ...
        {
            notice __x
"Removing the following permissions from {column} for group ID {group}: {perms}",
                column => $self->name,
                group  => $group_id,
                perms  => join(', ', @removed_permissions);
        }
        else
        {
            $self->schema->resultset('LayoutGroup')->search({
        ...
        {
            notice __x
"Adding the following permissions to {column} for group ID {group}: {perms}",
                column => $self->name,
                group  => $group_id,
                perms  => join(', ', @added_permissions);
        }
        else
        {
        ...
        if ($read_removed && !$options{report_only})
        {
            ...
            foreach my $sort (@sorts)
            {
                ...
                # another group
                $sort->delete
                    unless $self->user_id_can($sort->view->user_id, 'read');
            }
            ...
            foreach my $filter (@filters)
            {
                ...
                my $filtered =
                    _filter_remove_colid($self, $filter->view->filter);
...
{   my ($filter, $colid) = @_;
    ...
    if (my $rules = $filter->{rules})
    {
        ...
        @$rules = grep {
            _filter_remove_colid_decoded($_, $colid)
                && (!$_->{rules} || @{ $_->{rules} })
        } @$rules;
    }
...
{   my ($self, $values, %options) = @_;
    ...
    notice __x "Update: name_short from {old} to {new} for {name}",
        old  => $self->name_short,
        new  => $values->{name_short},
        name => $self->name
        if $report
        && ($self->name_short || '') ne ($values->{name_short} || '');
    ...
    notice __x "Update: group_display from {old} to {new} for {name}",
        old  => $self->group_display,
        new  => $values->{group_display},
        name => $self->name
        if $report
        && ($self->group_display || '') ne ($values->{group_display} || '');
    ...
    notice __x "Update: helptext from {old} chars to {new} chars for {name}",
        old  => length($self->helptext),
        new  => length($values->{helptext}),
        name => $self->name
    ...
    notice __x "Update: lookup_endpoint from {old} to {new} for {name}",
        old  => $self->lookup_endpoint,
        new  => $values->{lookup_endpoint},
        name => $self->lookup_endpoint
        if $report
        && ($self->lookup_endpoint || '') ne ($values->{lookup_endpoint} || '');
    $self->lookup_endpoint($values->{lookup_endpoint});
    notice __x "Update: lookup_group from {old} to {new} for {name}",
        old  => $self->lookup_group,
        new  => $values->{lookup_group},
        name => $self->lookup_group
        if $report
        && ($self->lookup_group || '') ne ($values->{lookup_group} || '');
...
{   my ($self, $values, %options) = @_;
    ...
    my $new_id =
        $values->{link_parent} ? $mapping->{ $values->{link_parent} } : undef;
    notice __x "Update: link_parent from {old} to {new} for {name}",
        old  => $self->link_parent,
        new  => $new_id,
        name => $self->name
