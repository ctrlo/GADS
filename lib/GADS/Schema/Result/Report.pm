...
{   my ($self, $record, $user) = @_;
    ...
    $pdf->text ('Last edited by '.$self->createdby->email.' on '.$record->created->format_cldr($format).' at '.$record->created->hms, size=>10);
    $pdf->text ('Report generated by '.$user->email.' on '.$now->format_cldr($format).' at '.$now->hms, size => 10);
