...
{   my ($self, $values, %options) = @_;
    ...
    notice __x "Update: initial_state from {old} to {new} for topic {name}",
        ...
        new  => $values->{initial_state},
        name => $self->name
        if $report
        && ($self->initial_state || '') ne ($values->{initial_state} || '');
    $self->initial_state($values->{initial_state});

    notice __x "Update: click_to_edit from {old} to {new} for topic {name}",
        old  => $self->click_to_edit,
        new  => $values->{click_to_edit},
        name => $self->name
        if $report && $self->click_to_edit != $values->{click_to_edit};
    $self->click_to_edit($values->{click_to_edit});

    notice __x
        "Update: prevent_edit_topic_id from {old} to {new} for topic {name}",
        old  => $self->prevent_edit_topic_id,
        new  => $values->{prevent_edit_topic_id},
        name => $self->name
        if $report
