...
sub graphs {
    ...
    $search->{graph_id} = { '!=' => [ -and => @$graphs ] }
      if @$graphs;
    $self->search_related( 'user_graphs', $search, { join => 'graph' } )
      ->delete;
}
...
sub permissions {
    ...
    foreach my $perm (qw/useradmin audit superadmin/) {
        ...
        if ( $user_perms{$perm} ) {
            ...
            $self->search_related( 'user_permissions',
                { permission_id => $pid } )->delete;
        }
    }
}
...
sub update_attributes {
    ...
    my $authentication =
      $self->result_source->schema->resultset('Authentication')->saml2_provider;
...
sub for_data_table {
    ...
    my $return = {
        ...
        'Last login' => {
            ...
            name   => 'Last login (GMT)',
            values =>
              [ $self->lastlogin ? $self->lastlogin->ymd : 'Never logged in' ],
        },
    };
    $return->{Title} = {
        type   => 'string',
