...
__PACKAGE__->has_many(
    "groups", "GADS::Schema::Result::Group",
    { "foreign.site_id" => "self.id" },
    { cascade_copy      => 0, cascade_delete => 0 },
);
...
__PACKAGE__->has_many(
    "teams", "GADS::Schema::Result::Team",
    { "foreign.site_id" => "self.id" },
    { cascade_copy      => 0, cascade_delete => 0 },
);

__PACKAGE__->has_many(
    "titles", "GADS::Schema::Result::Title",
    { "foreign.site_id" => "self.id" },
    { cascade_copy      => 0, cascade_delete => 0 },
);
...
{   my ($self, @fieldnames) = @_;
    ...
    $_->{editable} = $editable{ $_->{name} } || 0 foreach @fields;

    my $json = encode_json + { map { $_->{name} => $_->{editable} } @fields };
...
{   my $self = shift;
    ...
    my @fields = (
        ...
        },
        {
            ...
            type        => 'textarea',
            placeholder => $self->account_request_notes_placeholder
                || 'Notes',
            user_hidden => 1,
        },
    );
    ...
    push @fields,
        {
            ...
            type        => 'freetext',
            placeholder => $self->register_freetext1_placeholder
            || $self->register_freetext1_name,
        }
    ...
    push @fields,
        {
            ...
            type        => 'freetext',
            placeholder => $self->register_freetext2_placeholder
            || $self->register_freetext2_name,
        }
