...
{   my ($self, $file, %options) = @_;
    ...
    if (@invalid)
    {
        ...
        error __x
"The following column headings were found which are invalid: {invalid}. "
    ...
    my %titles =
        map { lc $_->name => $_->id } $schema->resultset('Title')->ordered->all;
    my %organisations = map { lc $_->name => $_->id }
        $schema->resultset('Organisation')->ordered->all;
    my %departments = map { lc $_->name => $_->id }
        $schema->resultset('Department')->ordered->all;
    my %teams =
        map { lc $_->name => $_->id } $schema->resultset('Team')->ordered->all;
    ...
    while (my $row = $csv->getline($fh))
    {
        ...
        my %values = (
            firstname => defined $user_mapping{forename}
            ? $row->[ $user_mapping{forename} ]
            : '',
            surname => defined $user_mapping{surname}
            ? $row->[ $user_mapping{surname} ]
            : '',
            email => defined $user_mapping{'email address'}
            ? $row->[ $user_mapping{'email address'} ]
            : '',
            freetext1 => defined $user_mapping{$freetext1}
            ? $row->[ $user_mapping{$freetext1} ]
            : '',
            freetext2 => defined $user_mapping{$freetext2}
            ? $row->[ $user_mapping{$freetext2} ]
            : '',
            title         => $title_id,
            organisation  => $org_id,
    ...
    if (@errors)
    {
        ...
        error __x
            "The upload failed with errors on the following lines: {errors}",
            errors => join '; ',
            @e;
    }
...
{   my ($self, $user) = @_;
    ...
    if (!$u)
    {
        $u = $self->create({
            ...
            pwchanged => $user->{pwchanged}
                && DateTime::Format::ISO8601->parse_datetime(
                    $user->{pwchanged}),
            deleted => $user->{deleted}
                && DateTime::Format::ISO8601->parse_datetime($user->{deleted}),
            ...
            created               => $user->{created}
                && DateTime::Format::ISO8601->parse_datetime($user->{created}),
        });
    }
