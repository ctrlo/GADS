...
{   my ($self, $user_id) = @_;
    ...
    my ($user_perms) =
        $perms_rs->all;    # The overall user. Only one due to query.
    my $return;
    if ($user_perms)       # Might not be any at all
    {
        foreach my $group (
            @{ $user_perms->{user_groups} })    # For each group the user has
        {
            foreach my $layout_group (@{ $group->{group}->{layout_groups} }
                )                               # For each column in that group
            {
                # Push the actual permission onto an array
                $return->{ $layout_group->{layout_id} }
                    ->{ $layout_group->{permission} } = 1;
            }
        }
    }
    return $return;
...
{   my $self = shift;
    ...
    {
        foreach
            my $group ($user_perms->user_groups)   # For each group the user has
        {
            foreach my $instance_group ($group->group->instance_groups
                )    # For each column in that group
            {
...
{   my $self = shift;
    ...
    {
        $overall->{$_} = 1 foreach @{ $self->layout_perms };
    }
    else
    {
...
my @all_rags =
    qw/danger attention warning advisory success undefined unexpected complete/;
...
{   my $self = shift;
    ...
    delete $all{ $_->rag } foreach $self->_rset->instance_rags;
...
{   my $self = shift;
    ...
    # Ensure fixed order
    [ grep { $_->enabled } map { $self->_rag_index->{$_} } @all_rags ];
}
...
{   my $self = shift;
    my @columns =
        map { $self->column($_->layout_id) } $self->_rset->alert_columns;
...
{   my $self = shift;
    ...
    {
        error __x
"Invalid short name for table \"{name}\": short names can only contain alphanumeric characters and the underscore",
...
{   my $self = shift;
    ...
    # Delete views
    my $views_rs = $self->schema->resultset('View')
        ->search({ instance_id => $self->instance_id });
...
{   my $self     = shift;
    ...
    foreach my $col (@internal)
    {
        ...
        try
        { # May have since been inserted by another process - unique constraint catches
...
{   my $self   = shift;
    ...
    my $instance_id_sql =
        $schema->resultset('Instance')->get_column('id')->as_query;
    my $cols_rs = $schema->resultset('Layout')->search(
        ...
        },
        {
            ...
            prefetch =>
                [ 'calcs', 'rags', 'link_parent', 'display_fields' ],
        },
    );
...
{   my $self = shift;
    [ map { $_->{id} }
            grep { $_->{instance_id} == $self->instance_id }
            @{ $self->use_layout->cols_db } ];
}
...
{   my $self = shift;
    !!$self->schema->resultset('Topic')
        ->search({ instance_id => $self->instance_id })->next;
}
...
{   my $self = shift;
    ...
    my %cols =
        map { $_ => 1 } $self->schema->resultset('LayoutDepend')->search(
            {
                instance_id => $self->instance_id,
            },
            {
    ...
    $cols{$_} = 1
        foreach $self->schema->resultset('LayoutDepend')->search(
            {
                instance_id => $self->instance_id,
            },
            {
...
{   my $self   = shift;
    my @return = map { +{ id => $_->id, label => $_->name, type => 'string', } }
        $self->all(exclude_internal => 1);
    encode_base64(encode_json \@return, '')
        ;    # Base64 plugin does not like new lines
}
...
{   my ($self, %options) = @_;
    ...
    @columns = grep { $_->return_type eq 'globe' } @columns
        if $options{is_globe};
    @columns = grep { $_->remember == $options{remember} } @columns
        if defined $options{remember};
    @columns = grep { $_->userinput == $options{userinput} } @columns
        if defined $options{userinput};
    @columns = grep { $_->has_cache } @columns if $options{has_cache};
    @columns = grep { $_->multivalue == $options{multivalue} } @columns
        if defined $options{multivalue};
    @columns = grep { $_->user_can('read') } @columns
        if $options{user_can_read};
    @columns = grep { $_->user_can('write') } @columns
        if $options{user_can_write};
    @columns = grep { $_->user_can('write_new') } @columns
        if $options{user_can_write_new};
    @columns =
        grep { $_->user_can('write_new') || $_->user_can('read') } @columns
        if $options{user_can_readwrite_new};
    @columns = grep { $_->user_can('write_existing') } @columns
        if $options{user_can_write_existing};
    @columns =
        grep { $_->user_can('write_existing') || $_->user_can('read') }
        @columns
        if $options{user_can_readwrite_existing};
    @columns = grep { $_->user_can('approve_new') } @columns
        if $options{user_can_approve_new};
    @columns = grep { $_->user_can('approve_existing') } @columns
        if $options{user_can_approve_existing};
...
{   my ($self, @columns) = @_;
    ...
    my %deps = map {
        my $id = $_->id;
        $_->id
            => # Allow fields depend on theirselves, but remove from dependencies to prevent recursion
            {
                map { $_ => 1 } grep $_ != $id,
                @{
                      $_->has_display_field
                    ? $_->display_field_col_ids
                    : $_->depends_on
                },
            }
    } @columns;
    ...
    foreach my $key (keys %deps)
    {
        ...
        error __x
"Calculated value or display condition recursive dependencies: {path}",
            path => $ret
            if $ret;
    }
...
{   my $self = shift;
    ...
    my %columns = map {
              $_->internal
            ? $_->name_short . $_->instance_id
            : $_->name_short => $_
    } grep { $_->name_short } @{ $self->use_layout->columns };
    \%columns;
}
...
{   my ($self, $values, %options) = @_;
    ...
    notice __x "Update: hide_in_selector from {old} to {new} for layout {name}",
        old  => $self->hide_in_selector,
        new  => $values->{hide_in_selector},
        name => $self->name
...
{   my ($self, $values, %options) = @_;
    ...
    {
        my $new_id = $values->{sort_layout_id}
            && $mapping->{ $values->{sort_layout_id} };
        notice __x "Update: sort_layout_id from {old} to {new} for {name}",
            old  => $self->sort_layout_id,
            new  => $new_id,
            name => $self->name
...
{   my $self = shift;
    ...
    $_->delete
        foreach
        reverse $self->all(order_dependencies => 1, include_hidden => 1);
