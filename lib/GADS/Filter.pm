...
# Need to compare as structures, as stringified JSON could be in different orders
        $self->_set_changed(1)
            if $self->has_value
            && !Compare($self->as_hash, decode_json_utf8($new),
                { ignore_hash_keys => ['column_id'] });
        $self->clear_as_hash;
...
has as_hash => (
    ...
    trigger => sub {
        my ($self, $new) = @_;
        $self->_set_changed(1)
            if $self->has_value && !Compare(decode_json_utf8($self->as_json),
                $new, { ignore_hash_keys => ['column_id'] });
        $self->clear_as_json;
...
sub decode_json_utf8 { decode_json(encode("utf8", shift)) }
...
{   my ($self, $filter, $tables) = @_;
    ...
    elsif (my $id = $filter->{id})
    {
        ...
        push @$tables,
            $filter;    # Keep as reference so can be updated by other functions
    }
}
...
{   my ($self, $single, $layout) = @_;
    ...
    elsif ($single->{value} && $single->{value} =~ /^\$([_0-9a-z]+)$/i)
    {
        ...
        else
        {
            ...
            $single->{value} =
                $col->numeric ? $datum->values->[0] : $datum->as_string;
