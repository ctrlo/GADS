...
{   my ($self, %options) = @_;
    ...
    foreach my $view (@{ $self->_view_limits })
    {
        if (my $filter = $view->filter)
        {
            ...
            if (keys %$decoded)
            {
                ...
                # effect
                push @search,
                    $self->_search_construct($decoded, $self->layout, %options,
                        ignore_perms => 1);
            }
        }
    }
    ...
    if (my $filter =
        $self->_view_limit_extra && $self->_view_limit_extra->filter)
    {
        my $decoded = $filter->as_hash;
        if (keys %$decoded)
        {
            my @s = $self->_search_construct($decoded, $self->layout, %options,
                ignore_perms => 1);

            # Get the user search criteria. As above we do not let permissions
            # affect these admin-defined views.
            @limit = @limit ? (-and => [ [@limit], @s ],) : @s;
        }
    }
...
{   my ($self, %options) = @_;
    if ($self->has_linked(%options))
    {
        ...
        return {
            linked => [
                {
                    "record_single$alt" => [ "record_later$alt", @tables, ]
                },
            ],
        };
    }
...
{   my ($self, $current_ids, $view) = @_;
    ...
    foreach my $user (@users)
    {
        ...
        if (keys %$decoded)
        {
            ...
            push @searches,
                $self->record_later_search(linked => 1, search => 1);
...
{   my ($self, $column, $value, %params) = @_;
    ...
    my @retrieve_columns =
        $params{retrieve_columns} && @{ $params{retrieve_columns} };
    ...
    my @rules = ({
        ...
        ,    # May need to use value ID instead of string as search
        operator => 'equal',
    });
...
{   my $self = shift;
    ...
    my @fields = (
        {
            type        => 'string',
            plural      => 'strings',
            index_field => 'strings.value_index'
        },
        ...
        { type => 'current_id', plural => '' }
        ,    # Empty string to avoid uninit warnings
    );
    ...
    foreach my $col ($self->layout->all(user_can_read => 1))
    {
        ...
        push @columns_can_view, @{ $col->curval_field_ids }
            ...
            ;    # Curval type needs all its columns from other layout
    }
    ...
    foreach my $field (@fields)
    {
        ...
        next
            if ($field->{type} eq 'int' || $field->{type} eq 'current_id')
            && $search_local !~ /^-?\d+$/;
...
# Need to get correct "value" number for search, in case it's been incremented through view_limits
        my $s =
              $field->{sub}
            ? $self->value_next_join(search => 1) . ".$value_field"
            : "$plural.$value_field";

        my $joins =
            $field->{type} eq 'current_id'    # Include joins for limited views
            ? { 'record_single' =>
                [ 'record_later', $self->jpfetch(search => 1), ] }
            : $field->{sub} ? {
                'record_single' => [
                    'record_later',
                    $self->jpfetch(search => 1),
        ...
        my @currents = $self->schema->resultset('Current')->search(
            { -and => \@search },
            {
                join => $joins,
                rows => 1001
                ,    # Ensure that we know whether we've exceeded 1000 limit
            },
        )->all;
     ...
     # instance ID different from current, therefore must be curval field result
                ...
                my $found = $self->schema->resultset('Current')->search(
                    { -and => \@search },
                    {
                        join => {
                            record_single => [
                                'record_later', 'curvals',
                                $self->jpfetch(search => 1),
                            ]
                        },
                    },
                );
                $found{$_} = 1 foreach $found->get_column('id')->all;
            }
            else
            {
                $found{ $current->id } = 1;
    ...
    $self->schema->resultset('Record')->search(
        {
            'me_created_date.id' => {
                -in =>
                    $self->schema->resultset('Current')->correlate('records')
            ...
            "" => $self->schema->resultset('Record')->search(
                {
                    'me_created.id' => {
                        -in => $self->schema->resultset('Current')
                            ->correlate('records')->get_column('id')
                            ->min_rs->as_query,
                    },
                },
                {
...
{   my ($self, %options) = @_;
    ...
    my $search_query =
        $self->search_query(search => 1, sort => 1, linked => 1, %limit)
    ...
    my $select = {
        join => [
            ...
            {
                'record_single' =>
                    [ # The (assumed) single record for the required version of current
                        $self->record_earlier
                        ? 'record_earlier'
                        : 'record_later'
                        , # The record after the single record (undef when single is latest)
                        $self->jpfetch(
                            search => 1,
                            sort   => 1,
                            linked => 0,
        ...
        distinct =>
            1,    # Otherwise multiple records returned for multivalue fields
        select    => [ 'me.id', ],
        '+select' => $self->_created_plus_select,
        '+as'     => [
            'record_created_date', 'record_created_user',
    ...
    $page = $self->pages
        ...
        && $page > 1
        && $page >
        $self->pages;    # Building page count is expensive, avoid if not needed
    ...
    $self->schema->resultset('Current')
        ->search([ -and => $search_query ], $select)->get_column('me.id');
}
...
{   my ($self, %options) = @_;
    ...
    {
        $search = {
            map { %$_ } $self->record_later_search(
                prefetch          => 1,
                sort              => 1,
                linked            => 1,
                group             => 1,
...
# Don't specify linked for fetching columns, we will get whataver is needed linked or not linked
        my @columns_fetch = $self->columns_fetch(
            sort              => 1,
            limit             => $limit,
            page              => $page,
            retain_join_order => 1
        );
        my $has_linked = $self->has_linked(
            prefetch => 1,
            sort     => 1,
            limit    => $limit,
            page     => $page
        );
        my $base = $self->record_name(
            prefetch => 1,
            sort     => 1,
            limit    => $limit,
            page     => $page
        ...
        foreach my $rec (@recs)
        {
            ...
            foreach my $key (keys %$rec)
            {
                ...
                {
                    my @existing =
                        ref $records->{$index_id}->{$key} eq 'ARRAY'
                        ? @{ $records->{$index_id}->{$key} }
                        : ($records->{$index_id}->{$key});
            ...
            push @retrieved_order, $index_id
                if $first_run
                && (!$last_current_id || $last_current_id != $current_id);
...
{   my ($self, %params) = @_;
    ...
    my @linked_ids;
    push @linked_ids,
        map { $_->linked_record_id } grep { $_->linked_record_id } @$records;
    ...
    foreach my $column (@{ $self->columns_retrieved_no })
    {
        ...
        if ($column->type eq 'curval')
        {
            ...
            push @cols,
                @{
                    $column->curval_fields_multivalue(
                ...
                )
            {
                ...
                push @{ $curval_fields->{ $column->field }->{ $_->field } },
                    $column->field;
            }
        }
        foreach my $col (@cols)
        {
            my $parent_field_this = $parent_field
                && $col->field ne $parent_field ? $parent_field : 'main';
            ...
            foreach my $loop (0 .. 1)
            {
                ...
                if (   !$col->fetch_with_record
                    && !$cols_done->{$parent_field_this}->{ $col->id })
                {
                    ...
                    next
                        if $self->separate_records_for_multicol
                        && $self->separate_records_for_multicol == $col->id;

                    my @retrieve_ids = $is_linked ? @linked_ids : @$record_ids;
                    foreach my $parent_curval_field (
                        @{
                            $curval_fields->{ $column->field }
                                ->{ $col->field }
                        }
                        )
                    {
                        ...
                        foreach my $rec (@$retrieved)
                        {
                            ...
                            {
                                my @vals =
                                    ref $rec->{$parent_curval_field} eq 'ARRAY'
                                    ? @{ $rec->{$parent_curval_field} }
                                    : $rec->{$parent_curval_field};
                                ...
                                # too low level.
                                local $GADS::Schema::Result::Record::REWIND =
                                    $self->rewind_formatted
                                    if $self->rewind;
                                my $search = {
                                    'me.id'           => \@value_ids,
                                    'record_later.id' => undef,
                                };
                                $search->{'record_single.created'} =
                                    { '<=' => $self->rewind_formatted }
                                    if $self->rewind;
                                my @curs =
                                    $self->schema->resultset('Current')
                                ...
                                {
                                    push @retrieve_ids,
                                        $current->get_column('record_id');
                                    $value_mapping->{ $current->get_column(
                                            'current_id') } =
                                        $current->get_column('record_id');
                                }
                                $_->{record_id} =
                                    $value_mapping->{ $_->{value} }
              ...
              # we don't do this.
                    ...
                    foreach my $val ($col->fetch_multivalues(
                        ...
                        rewind   =>
                            $self->rewind, # Would be better in a context object
                        already_seen => $already_seen,
                    ))
                    {
                        my $field = "field$val->{layout_id}";
                        next
                            if $cols_done->{$parent_field_this}
                            ->{ $val->{layout_id} };
                        $multi->{$parent_field_this}->{ $val->{record_id} }
                            ->{$field} ||= [];
                        push @{ $multi->{$parent_field_this}
                                ->{ $val->{record_id} }->{$field} }, $val;
                        $colsd{ $val->{layout_id} } = 1;
                    }
                    $cols_done->{$parent_field_this}->{$_} = 1
                        foreach keys %colsd
    ...
    foreach my $row (@$records)
    {
        ...
        foreach my $cv (keys %$curval_fields)
        {
            ...
            {
                foreach my $curval_field (
                    @{ $curval_fields->{$cv}->{$curval_subfield} })
                {
                    my @subs =
                        ref $record->{$curval_field} eq 'ARRAY'
                        ? @{ $record->{$curval_field} }
                        : $record->{$curval_field};
                    foreach my $subrecord (@subs)   # Foreach whole curval value
                    {
                        $subrecord->{record_id} or next;
                        $subrecord->{$curval_subfield} = $multi->{$curval_field}
                            ->{ $subrecord->{record_id} }->{$curval_subfield};
                    }
                }
            }
        }
        ...
        {
            $record->{$_} = $multi->{main}->{ $row->linked_record_id }->{$_}
                foreach keys %{ $multi->{main}->{ $row->linked_record_id } };
        }
    }
}
...
{   my $self = shift;
    ...
    if (!$self->is_group)    # Don't retrieve in chunks for group records
    {
            ...
            )
        {
            ...
            $end = @{ $self->_all_cids_store } - 1
                if @{ $self->_all_cids_store } - 1 < $end;
...
{   my $self = shift;
    ...
    $self->schema->resultset('Current')
        ->search([ -and => $search_query ], $select)->count;
}

# Cache for exists() function
has _exists_cache => (is => 'rw',);
...
{   my ($self, $value) = @_;
    ...
    !!$self->schema->resultset('Current')
        ->search([ -and => $search_query ], $select)->get_column('me.id')->next;
}
...
{   my $self = shift;
    ...
    my @child = $self->schema->resultset('Current')
        ->search([ -and => [@search_query] ], $select)->all;
    @child ? 1 : 0;
}
...
{   my $self = shift;
    ...
    foreach my $c (@columns)
    {
        ...
        $self->add_prefetch($c,
            include_multivalue => $self->separate_records_for_multicol);
...
{   my $self = shift;
    my %columns_retrieved =
        map { $_->id => undef } @{ $self->columns_retrieved_do };
    my @columns_retrieved_no =
        grep { exists $columns_retrieved{ $_->id } } $self->layout->all;
    \@columns_retrieved_no;
}

has separate_records_for_multicol => (is => 'rw',);
...
{   my $self = shift;
    ...
    if ($self->columns)
    {
        ...
        my @col_ids =
            map { ref $_ eq 'HASH' ? $_->{id} : $_ } @{ $self->columns };
    ...
    elsif (my $view = $self->view)
    {
        ...
        push @cols, map $self->layout->column($_), grep !$view_layouts{$_},
            $_->layout_id
            foreach @{ $view->groups };
...
{   my $self = shift;
    ...
    if ($self->view && !@{ $self->additional_filters })
    {
        ...
        @cols = grep !$self->has_group_col_id->{ $_->id }, @cols
            if $self->is_group;
        unshift @cols, grep $_->user_can('read'),
            map $self->layout->column($_), @{ $self->group_col_ids }
            if $self->is_group;
    }
...
{   my ($self, $c, $search_date, %options) = @_;
    ...
    elsif ($c->return_type =~ /date/)
    {
        ...
        my @f;
        my $sid =
            $options{parent_id} ? "$options{parent_id}_" . $c->id : $c->id;
        if (my $to = $self->to)
        {
            my $f = {
                id       => $sid,
                operator => $self->exclusive_of_to
                ? 'less'
                : 'less_or_equal',
        ...
        if (my $from = $self->from)
        {
            my $f = {
                id       => $sid,
                operator => $self->exclusive_of_from
                ? 'greater'
                : 'greater_or_equal',
...
{   my ($self, %options) = @_;
    ...
    for (1 .. 2)
    {
        ...
        if (@$search_date)
        {
            my @res = (
                $self->_search_construct(
                    { condition => 'OR', rules => $search_date }, $layout,
                    %options
                )
            );
        ...
        foreach my $additional (@{ $self->additional_filters })
        {
            ...
            my $f = {
                id       => $col->id,
                operator => $col->string_storage
                    || $additional->{is_text} ? 'contains' : 'equal',
                value       => $additional->{value},
                value_field => !$additional->{is_text}
                    && $col->value_field_as_index($additional->{value}),
            };
        ...
        if (my $view = $self->view)
        {
            ...
            if ($view->filter && !$self->search)
            {
                ...
                if (keys %$decoded)
                {
                    # Get the user search criteria
                    push @search,
                        $self->_search_construct($decoded, $layout, %options);
                }
            }
        }
...
{   my ($self, %options) = @_;
    ...
    if (!@sorts && $self->view && @{ $self->view->sorts })
    {
        foreach my $sort (@{ $self->view->sorts })
        {
            push @sorts, {
                id => $sort->layout_id
                    || $column_id->id,    # View column is undef for ID
...
{   my ($self, %options) = @_;
    ...
    my @sorts =
        $options{with_min} && $self->limit_qty
        ? @{ $self->_sorts_limit }
        : @{ $self->_sorts };
    ...
    foreach my $s (@sorts)
    {
        ...
        {
            my $random_sort =
                $self->schema->resultset('Current')->_rand_order_by;
        ...
        else
        {
            ...
            my @cols_link =
                $column->link_parent ? $column->link_parent->sort_columns : ();
            foreach my $col_sort (@cols_main)
            {
                ...
                else
                {
                    ...
                    $self->add_join(
                        $col_sort,
                        sort   => 1,
                        parent => $column->sort_parent
                    );
                }
                my $s_table = $self->table_name(
                    $col_sort,
                    sort => 1,
                    %options, parent => $column_parent || $column->sort_parent
                );
                my $sort_name;
                if (   $column->name_short
                    && $column->name_short eq '_created_user')
                {
                    ...
                    )    # Original column, not the sub-column ($col_sort)
                {
                    ...
                    my $link = $self->table_name(
                        ...
                        . $col_link->sort_field;
                    $sort_name =
                        $self->schema->resultset('Current')->helper_concat(
                            { -ident => $main },
                            { -ident => $link },
                        );
                }
    ...
    if ($self->limit_qty && $options{with_min} && @order_by)
    {
        ...
        @order_by = map {
            ...
            my $date =
                ...
                ? $self->schema->storage->datetime_parser->format_datetime(
                    $self->from || $self->to)
                : $self->schema->storage->datetime_parser->format_date(
                    $self->from || $self->to);
        ...
        if ($options{with_min})
        {
            ...
            @order_by =
                map { ref eq 'REF' ? $_ : +{ $func => { -ident => $_ } } }
                @order_by;
        }
        if ($self->limit_qty eq 'from')
        {
            return +{ -asc =>
                    $self->schema->resultset('Current')->helper_least(@order_by)
            };
        }
        else
        {
            return +{ -desc => $self->schema->resultset('Current')
                    ->helper_greatest(@order_by) };
        }
    }
...
{   my ($self, $filter, $layout, %options) = @_;
    ...
    my $ignore_perms =
           $options{ignore_perms}
        || $GADS::Schema::IGNORE_PERMISSIONS_SEARCH
    ...
    if (my $rules = $filter->{rules})
    {
        ...
        if ($filter->{previous_values})
        {
            ...
            my $records = GADS::Records->new(
                ...
                layout       => $self->layout,
                _view_limits => []
                , # Don't limit by view this as well, otherwise recursive loop happens
                previous_values => 1,
                view            => GADS::View->new(
                    filter      => $encoded,
                    instance_id => $self->layout->instance_id,
            ...
            my $match =
                $filter->{previous_values} eq 'negative' ? '-not_in' : '-in';
            return +{ 'me.id' =>
                    { $match => $records->_current_ids_rs->as_query } };
        }
        ...
        my $condition = $filter->{condition}
            && $filter->{condition} eq 'OR' ? '-or' : '-and';
...
# (used during filters on curval against current record values)
    my $filter_operator =
        $filter->{operator};    # Copy so as not to affect original hash ref
    $filter_operator =
        $filter_operator eq 'not_equal' ? 'is_not_empty' : 'is_empty'
    ...
    if ($column->return_type eq "daterange")
    {
        ...
        if (   $operator eq "!="
            || $operator eq "=")    # Only used for empty / not empty
        {
            push @conditions,
                {
        ...
        {
            error __x "Invalid operator {operator} for date range",
                operator => $operator;
        }
    }
    else
    ...
    if (   $operator eq '-like'
        || $operator eq
        '-not_like')    # Do not apply to "contains" for daterange
    {
        $vprefix = '%'
            if $filter_operator eq 'contains'
            || $filter_operator eq 'not_contains';
        $vsuffix = '%';
    }
    ...
    {
        my @original_values =
            ref $filter->{value} ? @{ $filter->{value} } : ($filter->{value});
         ...
         # than matching all or borking.
            ...
            if ($_ && $_ =~ /\[CURUSER(\.(ORG|DEPT|TEAM|ID|TITLE))?\]/)
            {
                ...
                elsif ($column->return_type eq "string")
                {
                    ...
                    my $curuser_org =
                          $curuser->organisation
                        ? $curuser->organisation->name
                        : '';
                    $_ =~ s/\[CURUSER\.ORG\]/$curuser_org/g;
                    my $curuser_dept =
                        $curuser->department ? $curuser->department->name : '';
                    $_ =~ s/\[CURUSER\.DEPT\]/$curuser_dept/g;
                    my $curuser_team =
                        $curuser->team ? $curuser->team->name : '';
                    $_ =~ s/\[CURUSER\.TEAM\]/$curuser_team/g;
                    my $curuser_title =
                        $curuser->title ? $curuser->title->name : '';
...
{   my ($self, $column, $condition, $default_value, $is_linked, %options) = @_;
    ...
    my $multivalue =
        $options{parent} ? $options{parent}->multivalue : $column->multivalue;
    ...
    if ($condition->{type} eq 'changed_after')
    {
        ...
        my $sub2 = $self->schema->resultset('Changed')->search(
            undef,
            {
                alias    => 'me_other2',
                group_by => 'id',
        ...
        return ('me.id' => [ { -in => $sub1 }, { -in => $sub2 } ]);
    }
    elsif ($multivalue
        && $condition->{type} eq 'not_equal'
        && !$previous_values)
    {
        ...
        my $records = GADS::Records->new(
            ...
            , # Don't limit by view this as well, otherwise recursive loop happens
            view => GADS::View->new(
                filter => { %{ $options{filter} }, operator => 'equal' }
                ,    # Switch
                instance_id => $self->layout->instance_id,
                layout      => $self->layout,
                schema      => $self->schema,
    ...
    {
        my $combiner =
            $condition->{type} =~ /(is_not_empty|not_equal|not_begins_with)/
            ? '-and'
            : '-or';
...
{   my ($self, %options) = @_;
    ...
    {    # We may have retrieved values other than the ones we want, for example
        ...
        if ($retrieved_count < 50)
        {
            ...
            for (my $r = $retrieved_count ;
                @items <= 50 && @over ; $r++)    #XXX 100 @items?
            {
                push @items, pop @over;
...
# invalid time and DateTime will bork.
    ...
    delete $_->{dt}    foreach @items;
    delete $_->{dt_to} foreach @items;
...
{   my ($self, $name) = @_;
    ...
    return $dbh->quote_identifier($table) . "."
...
{   my $self      = shift;
    ...
    foreach my $c (@cols)
    {
        push @cols_extra, map $self->layout->column($_), grep !$retrieved{$_},
            @{ $c->depends_on }
...
{   my $self = shift;
    ...
    my @columns =
        map  { +{ id => $_->id, column => $_, operator => $_->aggregate, } }
        grep { $_->aggregate } @{ $self->columns_render },
        @{ $self->columns_recalc_extra };

    return undef if !@columns;

    $self->_clear_cid_search_query_cache
        ;    # Clear any list of record IDs from previous results() call
    my $results = $self->_build_group_results(
        columns   => \@columns,
        is_group  => 1,
        aggregate => 1
    );
...
{   my ($self, %options) = @_;
    ...
    my $is_table_group =
        !$self->isa('GADS::RecordsGraph') && !$self->isa('GADS::RecordsGlobe');
    ...
    {
        my %view_group_cols =
            map { $_->layout_id => $_->parent_id } @{ $view->groups };
        @cols = map {
            +{
                id       => $_->id,
                column   => $_,
                operator => $_->numeric ? 'sum'
                : exists $view_group_cols{ $_->id } ? 'max'
                : 'distinct',
            ...
            push @parents,
                {
                    ...
                    group    => $col->{group},
                    drcol    => $self->dr_column
                    && $self->dr_column == $column->id,
                };
            $col->{parent} = $parent;
        }
    ...
    foreach my $col (@cols)
    {
        ...
        if (   ($column->multivalue || ($parent && $parent->multivalue))
            && !$col->{group}
            && !$col->{drcol})
        {
            ...
            if ($parent)
            {
                my $f_rs = $self->schema->resultset('Curval')->search(
                    {
                        'mecurval.record_id' => {
                            -ident =>
                                'record_single.id' # Match against main query's records
                        },
                    ...
                    {
                        alias => 'mecurval'
                        , # Can't use default "me" as already used in main query
                        join => {
                            'value' => {
                                'record_single_alternative' =>
                                    [ 'record_later', $column->tjoin, ],
                            },
                        },
                    },
                );
                ...
                else
                {
                    ...
                    panic __x
"Unexpected count distinct for curval sub-field {name} ({id})",
                        name => $column->name,
                        id   => $column->id;
                }
            }
            ...
            else
            {
                ...
                elsif ($self->isa('GADS::RecordsGraph')
                    || $self->isa('GADS::RecordsGlobe'))
                {
    ...
    {
        my $increment =
            $self->dr_interval . 's';    # Increment between x-axis values
        my $dr_col = $self->layout->column($self->dr_column)
            ;                            # The daterange column for x-axis
        my $field      = $dr_col->field;
        my $field_link = $dr_col->link_parent
            && $dr_col->link_parent->field;    # Related link field
        ...
        my $search =
            $self->search_query(search => 1, prefetch => 1, linked => 0);
        ...
        my $daterange_from =
            $self->from ? $self->from->clone : $self->_min_date(
        ...
        if ($daterange_from && $daterange_to)
        {
           ...
           # The literal CASE statement, which we reuse for each required period
            ...
            my $from_field_link =
                $field_link && $self->quote($field_link . ".from");
            my $to_field_link =
                $field_link && $self->quote($field_link . ".to");
            my ($dr_y_axis) = grep { $_->{id} == $self->dr_y_axis_id } @cols;
            my $col_val = $self->fqvalue(
                $dr_y_axis->{column},
                search   => 1,
                prefetch => 1
            );
            ...
            while ($pointer->epoch <= $daterange_to->epoch)
            {
                ...
                my $from = $self->schema->storage->dbh->quote(
                    $dt_parser->format_date(
                        $pointer->clone->add($increment => 1)
                    )
                );
                ...
                {
                    my $col_val_link = $self->fqvalue(
                        $dr_y_axis->{column}->link_parent,
                        linked   => 1,
                        search   => 1,
                        prefetch => 1
                    );
                    my $sum =
                        $dr_y_axis->{operator} eq 'count' ? 1 : $col_val_link;
                    my $casef =
                        $field_link
                        ? sprintf($case, $from, $from, $to, $to, $sum)
                        : sprintf($case, $from, $to, $sum);
    ...
    foreach (grep { $_->{group} } @cols)
    {
        ...
        if (my $pluck = $_->{pluck})
        {
            ...
            push @g,
                $self->schema->resultset('Current')->dt_SQL_pluck(
                    {
                        -ident =>
                        $self->fqvalue($col, search => 1, prefetch => 1)
                    },
                    'year'
                );
        ...
        else
        {
            if ($col->link_parent)
            {
                ...
                push @g,
                    $self->schema->resultset('Current')
                    ->helper_concat({ -ident => $main }, { -ident => $link },);
            }
            else
            {
                if ($_->{parent})
