...
after set_value => sub {
    ...
    my @ids = grep { $_ =~ /^[0-9]+$/ }
        @values;                 # Submitted curval IDs of existing records
    my @queries = grep { $_ !~ /^[0-9]+$/ }
        @values;    # New curval records or changes to existing ones
    ...
    if (@records)
    {
        ...
        my @queries = map {
            $_->as_query(exclude_curcommon => $options{allow_set_autocur})
        } grep { $_->new_entry } @records;
    ...
    {
        $self->changed(1);
        $self->column->validate($_, fatal => 1) foreach @ids;
...
{   my $self = shift;
    ...
    elsif ($self->column->type eq 'autocur'
        && !$self->values_as_records
        )    # Would be nice to abstract to autocur class
    {
        my $already_seen = Tree::DAG_Node->new({ name => 'root' });
        my @values =
            $self->column->fetch_multivalues([ $self->record->record_id ],
...
sub all_records
{   my $self   = shift;
    my @return = $self->column->ids_to_values(
        $self->all_ids,
        fatal  => 1,
        rewind => $self->record->rewind
    );
...
{   my $self = shift;
    ...
    {
        @return = map { $self->column->_format_row($_) }
...
{   my $self = shift;
    ...
    [
        $self->column->schema->resultset('Curval')->search({
            value     => { '!=' => undef },
            record_id => $self->record->record_id_old
...
{   my $self = shift;
    ...
    delete $old{ $_->current_id }
        foreach grep { !$_->new_entry } @{ $self->values_as_query_records };
    return [ keys %old ];
}
...
{   my $self = shift;
    my %ids =
        map { $_ => 1 } $self->oldvalue ? @{ $self->oldvalue->all_ids } : ();
    $ids{$_} = 1 foreach @{ $self->all_ids };
    [ keys %ids ];
}

# ids that have been added or removed
has ids_changed => (is => 'lazy',);
...
{   my $self = shift;
    ...
    my @records =
        ...
        ? @{ $self->_init_value_hash->{records} }
        : @{ $self->_records };
    $_->delete_current, $_->purge_current
        foreach grep { $_->is_draft } @records;
}
...
{   my $self = shift;
    ...
    foreach my $query (@{ $self->values_as_query })
    {
        ...
        grep { $_ !~ /^(?:csrf_token|submit|current_id|field[0-9]+)$/ }
            keys %$params

            # Unlikely to be a user error
            and panic __x "Invalid query string: {query}", query => $query;
        my @columns = $self->column->layout_parent->all(
            user_can_write => 1,
            userinput      => 1
        );
...
around 'clone' => sub {
    ...
    {
        my @copied = map { $_->{record}->clone; } @{ $self->values };
...
{   my $self   = shift;
    ...
    $return->{values} = [];
    $return->{parent_layout_identifier} =
...
{   my $self = shift;
    ...
    foreach my $v (@{ $self->values })
    {
        ...
        my $link =
            "/record/$v->{id}?oi=" . $self->column->refers_to_instance_id;
...
{   my $self = shift;
    ...
    foreach my $val (@{ $self->values })
    {
        ...
        # don't use
        delete $val->{id}
            if $val->{record}->new_entry || $val->{record}->is_draft;
        $val->{presentation} = $val->{record}
...
{   my ($self, %params) = @_;
    ...
    my @cols_need = grep $_->name_short && $fields->{ $_->name_short },
        @{
        $self->column->curval_fields_retrieve(
            all_fields        => 1,
            already_seen_code => $child
        )
        };
    ...
    my @values = map {
        ...
        +{
            id =>
                int $_->current_id,  # Ensure passed to Lua as number not string
            field_values => $vals,
        }
