...
{   my ($self, %options) = @_;
    ...
    my %permissions =
        ...
        : $options{new}                       ? (user_can_readwrite_new => 1)
        :   (user_can_readwrite_existing => 1);

    my @columns = $self->layout->all(
        sort_by_topics => 1,
        can_child      => $options{child},
        %permissions, exclude_internal => 1
    );
...
{   my ($self, %options) = @_;
    ...
    my @columns =
        ...
        : $options{group}         ? @{ $self->columns_render }
        : $options{purge}         ? $self->layout->column_id
        :                           @{ $self->columns_render };
    ...
    foreach my $col (@columns)
    {
        if ($col->has_display_field)
        {
            ...
            {
                my ($seen) =
                    grep { $_ == $display_field_id } keys %$this_indent;
                if ($seen || ($previous && $display_field_id == $previous->id))
                {
                    $indent->{ $col->id } =
                        $seen && $indent->{$seen} ? ($indent->{$seen} + 1) : 1;
    ...
    my @presentation_columns =
        $self->presentation_map_columns(%options, columns => \@columns);
    my @topics = $self->get_topics(\@presentation_columns);

    my $version_datetime_col =
        $self->layout->column_by_name_short('_version_datetime');
    ...
    {
        $return->{version_user} =
            $self->layout->column_by_name_short('_version_user')->presentation(
                datum_presentation => $self->createdby->presentation,
                %options
            );
        $return->{version_datetime} = $version_datetime_col->presentation(
            datum_presentation =>
                $self->fields->{ $version_datetime_col->id }->presentation,
            %options
        );
        $return->{created_user} = $created_user_col->presentation(
            datum_presentation =>
                $self->fields->{ $created_user_col->id }->presentation,
            %options
        );
        $return->{created_datetime} = $created_datetime_col->presentation(
            datum_presentation =>
                $self->fields->{ $created_datetime_col->id }->presentation,
            %options
        );
    }
