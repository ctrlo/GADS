...
has group_col_operator => (is => 'lazy',);
...
has color_col_operator => (is => 'lazy',);
...
has is_group => (is => 'lazy',);

sub _build_is_group
{   my $self = shift;
    return 1
        if $self->has_color_col || $self->group_col || $self->has_label_col;
    return 0;
}

has _columns => (is => 'lazy',);
...
{   my $self = shift;
    ...
    push @extra,
        {
        col      => $self->group_col,
        parent   => $self->group_col_parent,
        operator => $self->group_col_operator,
        group    => !$self->group_col->numeric
        }
        if ($self->group_col);
    push @extra,
        {
        col      => $self->color_col,
        parent   => $self->color_col_parent,
        operator => $self->color_col_operator,
        group    => !$self->color_col->numeric
        }
        if ($self->color_col);
    push @extra,
        {
        col    => $self->label_col,
        parent => $self->label_col_parent,
        group  => !$self->label_col->numeric
        }
        if $self->label_col;
    ...
    if (my $view_id = $records->view && $records->view->id)
    {
        ...
        if (!$self->is_group)
        {
            push @extra,
                map { +{ col => $_ } }
                grep { !$existing{ $_->id } } @{ $records->columns_render };
        }
    ...
    {
        push @extra,
            map { +{ col => $_ } } $records->layout->all(user_can_read => 1);
    }

    if ($self->is_group)
    {
        $_->{group} = 1
            foreach grep { $_->{col}->return_type eq 'globe' } @extra;
    }
        ...
        else
        {
            ...
            foreach my $column (@{ $self->_columns })
            {
                ...
                else
                {
                    ...
                    push @titles, { col => $column, value => $d->as_string }
                        if $d->as_string;
                }
            }
    ...
    foreach my $country (keys %countries)
    {
        ...
        foreach my $item (@items)
        {
            ...
            if ($self->has_label_col)
            {
                ...
                {
                    $values->{label_sum} ||= 0;
                    $values->{label_sum} += $item->{value_label}
                        if $item->{value_label};
                    push @{ $values->{group_sums} },
                        {
                            text => $item->{value_group},
                            sum  => $item->{value_label}
                        }
                        if $self->group_col;
                }
                else
                {
                    ...
                    $values->{label_text}->{ $label || '_count' } +=
                        $item->{id_count};
                }
            }
            ...
            if ($self->has_color_col)
            {
                ...
                {
                    $values->{color_sum} ||= 0;
                    $values->{color_sum} += $item->{value_color}
                        if $item->{value_color};
                    $values->{group_sums} ||= [];
           ...
           # Add individual group totals, if not already added in previous label
                    push @{ $values->{group_sums} },
                        {
                            text => $item->{value_group},
                            sum  => $item->{value_color}
                        }
                        if $self->group_col
                        && !(  $self->label_col
                            && $self->color_col_id eq $self->label_col_id);
                }
                else
                {
                    $values->{color_text}->{ $item->{value_color} } ||= 0;
                    $values->{color_text}->{ $item->{value_color} } +=
                        $item->{id_count};
                    $values->{color} =
                         !$values->{color} || $values->{color} eq $item->{color}
                        ? $item->{color}
                        : 'grey';
                }
            }
            ...
            if ($self->group_col)
            {
                ...
                {
                    $values->{group_sum} ||= 0;
                    $values->{group_sum} += $item->{value_group}
                        if $item->{value_group};
                }
                else
                {
                    $values->{group_text}->{ $item->{value_group} } ||= 0;
                    $values->{group_text}->{ $item->{value_group} } +=
                        $item->{id_count};
                }
            }
        ...
        my $hover = $self->is_choropleth
            ...
            map {
                      $_ eq '_count'
                    ? $values->{label_text}->{$_}
                    : "$_: $values->{label_text}->{$_}"
            } keys %{ $values->{label_text} }
            )
        ...
        $r->{label} =
               $self->group_col
            && $self->label_col && $self->label_col->numeric ? $group_sums
            : $self->label_col
            && $self->label_col->numeric ? $values->{label_sum}
            : join(
            '<br>',
            map {
                      $_ eq '_count'
    ...
    return {
        data   => \@return,
        params => {
            view_id => $self->records_options->{view}
                && $self->records_options->{view}->id,
            layout_identifier =>
                $self->records_options->{layout}->identifier,
            globe_fields => [
                map $_->{col}->field,
                grep $_->{col}->return_type eq 'globe', @extra
            ],
            default_view_limit_extra_id =>
                $self->records_options->{layout}->default_view_limit_extra_id,
        },
    };
}
...
{   my ($self, $column, $line) = @_;
    ...
    my $text = $column->format_value(map { $line->get_column($_->field) }
