...
{   my ($self, $column, %options) = @_;
    ...
    panic __x "Link parent of field ID {id} is from same table as field itself",
        id => $column->id
        if $column->link_parent
        && $column->link_parent->instance_id == $column->instance_id;
    ...
    my $prefetch = (
        $column->fetch_with_record
            || (   $options{include_multivalue}
                && $options{include_multivalue} == $column->id)
        )
        && $options{prefetch};
    ...
    foreach my $j ($self->_all_joins_recurse(@{ $self->_jp_store }))
    {
        trace __x "Checking join {field}", field => $j->{column}->field
            if $debug;
        if (($key && ref $j->{join} eq 'HASH' && Compare($toadd, $j->{join}))
            || $toadd eq $j->{join})
        {
            ...
            if (_compare_parents($options{parent}, $j->{parent}))
            {
                ...
                if ($column->is_curcommon && $prefetch)
                {
                    ...
                    my ($daughter) =
                        grep { $_->name == $column->id } $tree->daughters;
...
# Never prefetch multivalue columns, which can result in huge numbers of rows being retrieved.
        ...
        drcol    => $options{drcol}
        ,    # Whether it's used as a daterange column on a graph x-axis
        column => $column,
...
{   my ($self, %options) = @_;
    ...
    my $count =
        $options{no_current}
        ? 0
        : 1;    # Always at least one if joining onto current
    ...
    foreach my $li (
        0 .. !!$include_linked)    # Only do second loop if linked requested
    {
        ...
        foreach ($self->_all_joins(%options, linked => undef))
        {
            ...
            {
                my $curcommon_id =
                    $is_curcommon ? $_->{column}->id : $_->{parent}->id;
                if (
                    (
                           $options{search}
                        && $_->{search}
    ...
    for (1 .. $count)
    {
        ...
        push @search, { "record_later$alt$id.current_id" => undef, };
    }
    @search;
}
...
{   my ($self, %options) = @_;
    ...
    if ($options{limit} && @$joins)
    {
        ...
        push @return, grep { $_->{search} } @joins[ 0 .. $offset - 1 ]
            if $options{search};
        push @return, grep { $_->{sort} } @joins[ 0 .. $offset - 1 ]
            if $options{sort};
        push @return, @joins[ $offset .. $end ];
        push @return, grep { $_->{search} } @joins[ $end + 1 .. @joins - 1 ]
            if $options{search};
        push @return, grep { $_->{sort} } @joins[ $end + 1 .. @joins - 1 ]
            if $options{sort};
    }
    else
    {
...
{   my ($self, %params) = @_;
    ...
    if (
        ...
        || (   $options->{extra_column}
            && $join->{column}->id == $options->{extra_column}->id)
        )
    {
            ...
            push @$return,
                {
                    ...
                    column => $join->{column},
                    join   =>
                    $join->{column}->make_join(map { $_->{join} } @children),
                    search    => $join->{search},
                    sort      => $join->{sort},
                    group     => $join->{group},
                    drcol     => $join->{drcol},
...
{   my ($self, %options) = @_;
    ...
    foreach my $jp ($self->_jpfetch(prefetch => 1, %options))
    {
        ...
        push @prefetch, { $column->field . ".$_" => "$table.$_" }
            foreach @values;    # unless $column->is_curcommon;
        push @prefetch, $column->field . '.child_unique'
            if $column->userinput;
        if ($jp->{children} && @{ $jp->{children} })
        {
            my $rec_single_name =
                $self->record_name(%options, prefetch => 1, column => $column);
            push @prefetch,
                { $column->field . ".record_id" => "$rec_single_name.id" };
...
{   my ($self, %options) = @_;
    ...
    foreach my $c (@store)
    {
        ...
        next
            if !@{ $c->{children} }
            ;    # No record_singles unless fields selected from curval
        $count++;
        return _record_name_by_count($count)
...
{   my ($self, $column, %options) = @_;
    ...
    {
        trace __x "Looking in the store for join number for column {id}",
            id => $column->id
            if $debug;
    }
    ...
    $options{parent} = $options{parent}->id
        if $options{parent};    # Prevent dumping of whole object
    $options{extra_column} = $options{extra_column}->id
        if $options{extra_column};
    panic
        "Unable to get join number: column $cid hasn't been added for options "
        . Dumper(\%options);
}
...
{   my ($needle, $jp, $stash, %options) = @_;
    ...
    if (ref $jp->{join} eq 'HASH')
    {
        ...
        trace __x "This join is a hash with key:{key} and value: {value}",
            key   => $key,
            value => Dumper($value)
            if $debug;
        trace __x "We are looking for value being equal to {needle}",
            needle => $needle->sprefix
            if $needle && $debug;
        if (($options{find_value} && $value eq 'value')
            || $needle->sprefix eq $value)
        {
            ...
            $stash->{$value}++;
            if ($jp->{parent}
                && !$stash->{parents_included}->{ $jp->{parent}->id })
            {
                if (   $jp->{parent}->type eq 'curval'
                    || $jp->{parent}->type eq 'filval')
                {
            ...
            if (  !$options{find_value}
                && $needle->field eq $key
                && _compare_parents($options{parent}, $jp->{parent}))
            {
    ...
    else
    {
        ...
        if (!$options{find_value} && $needle->sprefix eq $jp->{join})
        {
            ...
            if (
                ...
                && (   $needle->sprefix ne 'current'
                    || $needle->id == $jp->{column}->id)
                )
            {
...
{   my ($self, $column, %options) = @_;
    ...
    my $value_field =
        $as_index ? $column->value_field_as_index : $column->value_field;
    "$tn." . $value_field;
}
