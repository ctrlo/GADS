...
{   my ($self, $layout, $current_ids) = @_;
    ...
    my @view_ids = $self->schema->resultset('View')->search(
        $search,
        {
            join     => [ 'alerts', 'filters' ],
            group_by =>
                'me.id',    # Remove same view ID across multiple alerts
        },
    )->get_column('id')->all;
    ...
    my $records_count = GADS::Records->new(
        columns => []
        ,    # Otherwise all columns retrieved during search construct
        schema => $self->schema,
        layout => $layout,
    ...
    if (my @views = map { $views->view($_) } @view_ids)
    {
        ...
        foreach my $view (@views)
        {
            my $records = GADS::Records->new(
                ...
                layout => $layout,
                user   =>
                    undef,    # Alerts should not be affected by current user
            );
 ...
 # all records. Therefore don't specify (the potentially thousands) current_ids.
            ...
            push @original, $self->schema->resultset('AlertCache')->search(
                $search,
                {
                    select => [
                        { max => 'me.current_id' },
                    ...
                    join     => 'view',
                    group_by =>
                        [ 'me.view_id', 'me.user_id', 'me.current_id' ]
  ...
  # of changed columns and records, and search the cache.
    ...
    my @caches;
    $search = {
        'alert_caches.layout_id' =>
            $self->columns,    # Columns that have changed
    ...
    while ($i < @$current_ids)
    {
        ...
        unless (@$current_ids == $total_records)
        {
            my $max = $i + 499;
            $max = @$current_ids - 1 if $max >= @$current_ids;
            $search->{'alert_caches.current_id'} =
                [ @$current_ids[ $i .. $max ] ];
        }
        push @caches,
            $self->schema->resultset('View')->search(
                $search,
                {
  ...
  # We iterate through each of the alert's caches, sending alerts where required
        ...
        foreach my $alert_cache ($view->alert_caches)
        {
            ...
            foreach my $alert (@alerts)
            {
                ...
                # why we probably can't do all these with one call to populate()
                    try {
                        $self->schema->resultset('AlertSend')->create($write)
                    };
                ...
                else
                {
                    ...
                    push @{ $send_now->{ $alert->user_id }->{col_ids} },
                        $col_id;
                    push @{ $send_now->{ $alert->user_id }->{cids} },
                        $alert_cache->current_id;
                }
            }
        }
        foreach my $a (values %$send_now)
        {
            ...
            my @cids     = @{ $a->{cids} };
            $self->_send_alert('changed', \@cids, $view, [ $a->{user} ],
                \@colnames);
        }
    }
...
{   my ($self, $action, @items) = @_;
    ...
    foreach my $item (@items)
    {
        ...
        $self->_send_alert($action, [ $item->{current_id} ],
            $item->{view}, \@users)
            if @users;
    }
}
...
has alert_description => (is => 'lazy',);

sub _build_alert_description
{   my $self = shift;
    GADS::AlertDescription->new(schema => $self->schema,);
}
...
{   my ($self, $action, $current_ids, $view, $users, $columns) = @_;
    ...
    foreach my $user (@$users)
    {
        ...
        if ($action eq "changed")
        {
            ...
            $text =
"The following items were changed for $description: $cnames\n\n";
            $text .= "Links to the records are as follows:\n";
            $html =
                "<p>The following items were changed for $link: $cnames</p>";
        }
        elsif ($action eq "arrived")
        {
            $text =
qq(New items as follows have appeared in the view "$view_name": $description\n\n);
            $text .= "Links to the new items are as follows:\n";
            $html =
"<p>New items as follows have appeared in the view &quot;$view_name&quot;: $link</p>";
        }
        elsif ($action eq "gone")
        {
            $text =
qq(Items as follows have disappeared from the view "$view_name": $description\n\n);
            $text .= "Links to the removed items are as follows:\n";
            $html =
"<p>Items as follows have disappeared from the view &quot;$view_name&quot;: $link</p>";
        }
