...
use Math::Random::ISAAC::XS
    ;    # Make Dancer session generation cryptographically secure
...
schema->exception_action(sub {
    die $_[0]
        if $_[0] =~ /^Unable to satisfy requested constraint/;    # Expected
...
set behind_proxy => config->{behind_proxy}
    ;    # XXX Why doesn't this work in config file
...
GADS::SchemaInstance->instance(schema => schema,);
...
config->{plugins}->{'Auth::Extensible'}->{realms}->{dbic}->{user_as_object}
    or panic
    "Auth::Extensible DBIC provider needs to be configured with user_as_object";
...
hook before => sub {
    ...
    else
    {
        ...
        {
            trace __x
                "Unknown host: {host}. Redirecting to configured host: {site}",
                host => request->base->host,
                site => $site->host;
            my $uri = request->base;
            $uri->host($site->host);
    ...
    if (request->is_post && request->path !~ m!^(/api/token|/print|/saml)$!)
    {
        ...
        my $token;
        if (request->content_type eq
            'application/json')    # Try in body of JSON
        {
            my $body = try { decode_json(request->body) };
            $token = $body->{csrf_token} if $body;
        }
        $token ||= query_parameters->get('csrf-token')
            || body_parameters->get('csrf_token');
        error __x "csrf-token missing for uri {uri}", uri => request->uri
            if !$token;
        error __x
"The CSRF token is invalid or has expired. Please try reloading the page and making the request again."
    ...
    {
        my $instances =
            GADS::Instances->new(schema => schema, user => $user);
        var 'instances' => $instances;

        if (my $layout_name = route_parameters->get('layout_name'))
        {
            if (my $layout = var('instances')
                ->layout_by_shortname($layout_name, no_errors => 1))
            {
                var 'layout' => $layout;
            }
        }
    ...
    if (logged_in_user)
    {
        if (config->{gads}->{aup})
        {
            ...
            if (my $aup_date = $user->aup_accepted)
            {
                ...
                $aup_accepted =
                    $aup_date_dt
                    && DateTime->compare($aup_date_dt,
                        DateTime->now->subtract(months => 12)) > 0;
            }
            redirect '/aup'
                unless $aup_accepted || request->uri =~ m!^/aup!;
        }

        if (config->{gads}->{user_status} && !session('status_accepted'))
        {
            # Redirect to user status page if required and not seen this session
            redirect '/user_status'
                unless request->uri =~ m!^/(user_status|aup)!;
        }
        elsif (logged_in_user_password_expired && !session('is_sso'))
        {
            ...
            forwardHome(
                {
                    danger =>
"Your password has expired. Please use the Change password button
                below to set a new password."
                },
                'myaccount'
                )
                unless request->uri eq '/myaccount'
                || request->uri eq '/logout';
        }

        response_header "X-Frame-Options" => "DENY"    # Prevent clickjacking
            unless request->uri eq
            '/aup_text'    # Except AUP, which will be in an iframe
            || request->path eq '/file'
            ;    # Or iframe posts for file uploads (hidden iframe used for IE8)
   ...
   # used as hashrefs themselves, so prevent trying to access non-existent hash.
        ...
        elsif (!$persistent->{instance_id})
        {
            $persistent->{instance_id} =
                var('instances')->all->[0]->instance_id
        ...
        notice __
"You do not have permission to access any part of this application. Please contact your system administrator."
            if !@{ var('instances')->all };
...
hook before_template => sub {
    ...
    $tokens->{scheme} ||=
        request->scheme;    # May already be set for phantomjs requests
    ...
    if (
           $user
        && $layout
        && (   $layout->user_can('approve_new')
            || $layout->user_can('approve_existing'))
        )
    {
    ...
    if (logged_in_user)
    {
     ...
     # Somehow this sets the instance_id session if no persistent session exists
        ...
        $tokens->{user_can_edit} =
            $layout && $layout->user_can('write_existing');
        $tokens->{user_can_create} =
            $layout && $layout->user_can('write_new');
        $tokens->{show_link} = rset('Current')->next ? 1 : 0;
        $tokens->{layout}    = $layout;
        $tokens->{v} =
            current_view(logged_in_user, $layout);    # View is reserved TT word
    }
    ...
    {
        notice __x
            "You are currently viewing, editing and creating views as {name}",
...
{   forwardHome() if !var('site')->remember_user_location;
    ...
    {
        my $instances =
            GADS::Instances->new(schema => schema, user => logged_in_user);
...
get '/' => require_login sub {
    ...
    my $params = {
        readonly => $dashboard->is_shared
            && !$user->permission->{superadmin},
        dashboard       => $dashboard,
        dashboards_json =>
            schema->resultset('Dashboard')->dashboards_json(%params),
        page                                => 'index',
    ...
    if (my $download = param('download'))
    {
        ...
        {
            my $pdf =
                _page_as_mech('index', $params, pdf => 1)->content_as_pdf(
                    paperWidth  => 16.5,
                    paperHeight => 11.7    # A3 landscape
                );
...
post '/saml' => sub {
    ...
    my $callback =
        $saml->callback(saml_response => body_parameters->get('SAMLResponse'),);

    my $authentication =
        schema->resultset('Authentication')->find(session 'authentication_id')
        or error "Error finding authentication provider";

    my $username = $callback->{nameid};
    my $user =
        schema->resultset('User')->active->search({ username => $username })
        ->next;

    if (!$user)
    {
        my $msg = $authentication->user_not_found_error;
        return forwardHome({ danger => __x($msg, username => $username) },
            'login?password=1');
    }
...
get '/login/denied' => sub {
    forwardHome({
        danger => "You do not have permission to access this page" });
};
...
'An email has been sent to your email address with a link to reset your password'
                    ...
                    { is_fatal => 0 },
                    ERROR =>
'Failed to send a password reset link. Did you enter a valid email address?'
                    );
                report
                    INFO => __x
"Password reset requested for non-existant username {username}",
                    username => $username
        ...
        {
            $error =
'Please enter an email address for the password reset to be sent to';
            $error_modal = 'resetpw';
        }
    }
    ...
    if (defined param('signin'))
    {
        ...
        my $lastfailf =
            schema->storage->datetime_parser->format_datetime($lastfail);
        ...
        my ($success, $realm) =
            !$fail && authenticate_user($username, params->{password});
        if ($success)
        {
        ...
        else
        {
            ...
            if ($user)
            {
                ...
                report { to => 'syslog' },
                    INFO => __x
"debug_login set - failed username \"{username}\", password: \"{password}\"",
...
any [ 'get', 'post' ] => '/register' => sub {
    ...
    {
        error __
            "Self-service account requests are not enabled on this site"
        ...
        if ($users->user_exists($params->{email}))
        {
            ...
            my $user       = schema->resultset('User')
                ->active->search({ username => $params->{email} })->next;
...
"Your account request has been received successfully"
            ...
            $audit->login_change(
"Account request for $params->{email}. Account already existed, resending welcome email."
            );
            return forwardHome(
                {
                    success =>
        ...
        else
        {
            ...
            {
                $audit->login_change(
                    "New user account request for $params->{email}");
                return forwardHome(
                    {
                        success =>
"Your account request has been received successfully"
...
any [ 'get', 'post' ] => '/myaccount/?' => require_login sub {
    ...
    if (param 'newpassword')
    {
        ...
        {
            forwardHome(
                { danger => "The existing password entered is incorrect" },
                'myaccount'
            );
        }
    }
    ...
    if (param 'submit')
    {
        ...
        {
            return forwardHome(
                { success => "The account details have been updated" },
                'myaccount');
        }
    }
...
my $default_email_welcome_text =
    ($new_account && $new_account->{body}) || <<__BODY;
...
any [ 'get', 'post' ]                           => '/group_overview/' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        template 'layouts/page_overview_name_only' => {
            ...
            page_title       => "Group",
            page_description =>
"Groups are the basis for LinkSpace’s fine-grained access control. Users can be allocated to any number of groups. You then set the group permissions for every field you create to control what fields users can view or edit.",
    ...
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        template 'layouts/page_save_name_only' => {
            ...
            item => {
                type        => "group",
                description =>
"In this window you can create a permission group. Under table and field management you can define permissions and functions available to this group.",
...
any [ 'get', 'post' ]                           => '/settings/?' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        template 'admin/admin_settings' => { page => 'system_settings', };
    };

any [ 'get', 'post' ]                 => '/settings/default_welcome_email/' =>
    require_any_role [qw/superadmin/] => sub {
        forwardHome(
            {
                danger => "You do not have permission to manage system settings"
            },
            ''
...
any [ 'get', 'post' ] => '/settings/user_editable_personal_details/' =>
    require_any_role [qw/superadmin/] => sub {
        forwardHome(
            {
                danger => "You do not have permission to manage system settings"
        ...
        if (param 'update')
        {
            if (
                process(sub {
                    $site->update_user_editable_fields(
                        body_parameters->get_all('user_editable'));
                })
            )
            {
                return forwardHome(
...
any [ 'get', 'post' ] => '/settings/title_overview/' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        if (my $delete_id = param('delete'))
        {
            ...
            if (process(sub { $title->delete_title }))
            {
                return forwardHome(
                    {
                        success =>
                        "The $title_name has been deleted successfully"
                    },
        ...
        template 'layouts/page_overview_name_only' => {
            ...
            page_description =>
"In this window you can list the ${title_name}s that you want to assign users to. You can update the existing items or add new ones. Changes in here will impact all users currently assigned if you delete or edit a value.",
...
any [ 'get', 'post' ]                           => '/settings/title_add/' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        if (body_parameters->get('submit'))
        {
            ...
            if (process(sub { $title->insert_or_update }))
            {
                return forwardHome(
                    {
                        success =>
                        "The $title_name has been created successfully"
                    },
                    'settings/title_overview/'
                );
            }
        ...
        $title->{description} =
        "In this window you can add a $title_name to assign to users.";
...
any [ 'get', 'post' ]                           => '/settings/title_edit/:id' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        if (body_parameters->get('submit'))
        {
            ...
            if (process(sub { $title->insert_or_update }))
            {
                return forwardHome(
                    {
                        success =>
                        "The $title_name has been updated successfully"
                    },
                    'settings/title_overview/'
                );
            }
        ...
        $title->{type} = $title_name;
        $title->{description} =
"In this window you can edit a ${title_name}. Changes will impact all users currently assigned if you delete or edit a value.";
        $title->{back_url}    = "${base_url}settings/title_overview/";
        $title->{field_label} = ucfirst($title_name);
...
any [ 'get', 'post' ] => '/settings/organisation_overview/' =>
    require_any_role [qw/useradmin superadmin/] => sub {

        my $organisation_name = lcfirst var('site')->organisation_name;
        my $organisation_name_plural =
        lcfirst var('site')->organisation_name_plural;

        if (my $delete_id = param('delete'))
        {
            my $organisation =
            schema->resultset('Organisation')->find($delete_id);
            ...
            if (process(sub { $organisation->delete_organisation }))
            {
                return forwardHome(
                    {
                        success =>
                        "The $organisation_name has been deleted successfully"
                    },
        ...
        template 'layouts/page_overview_name_only' => {
            ...
            page_description =>
"In this window you can list the parts of the $organisation_name that you want to assign users to. You can update the existing items or add new ones. Changes in here will impact all users currently assigned if you delete or edit a value.",
...
any [ 'get', 'post' ] => '/settings/organisation_add/' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        if (body_parameters->get('submit'))
        {
            ...
            if (process(sub { $organisation->insert_or_update }))
            {
                return forwardHome(
                    {
                        success =>
                        "The $organisation_name has been created successfully"
                    },
                    'settings/organisation_overview/'
                );
            }
        ...
        "In this window you can add a $organisation_name to assign to users.";
        $organisation->{back_url} =
        "${base_url}settings/organisation_overview/";
...
any [ 'get', 'post' ] => '/settings/organisation_edit/:id' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        my $id                = route_parameters->get('id');
        ...
        if (body_parameters->get('submit'))
        {
            ...
            if (process(sub { $organisation->insert_or_update }))
            {
                return forwardHome(
                    {
                        success =>
                        "The $organisation_name has been edited successfully"
                    },
                    'settings/organisation_overview/'
                );
            }
        ...
        $organisation->{type} = $organisation_name;
        $organisation->{description} =
"In this window you can edit a ${organisation_name}. Changes will impact all users currently assigned if you delete or edit a value.";
        $organisation->{back_url} =
        "${base_url}settings/organisation_overview/";
        $organisation->{field_label} = ucfirst($organisation_name);
...
any [ 'get', 'post' ] => '/settings/department_overview/' =>
    require_any_role [qw/useradmin superadmin/] => sub {

        my $department_name = lcfirst var('site')->department_name;
        my $department_name_plural =
        lcfirst var('site')->department_name_plural;
        ...
        if (my $delete_id = param('delete'))
        {
            ...
            if (process(sub { $department->delete_department }))
            {
                return forwardHome(
                    {
                        success =>
                        "The $department_name has been deleted successfully"
                    },
                    'settings/department_overview/'
                );
            }
        }
        ...
        template 'layouts/page_overview_name_only' => {
            ...
            page_description =>
"In this window you can list the $department_name that you want to assign users to. You can update the existing items or add new ones. Changes in here will impact all users currently assigned if you delete or edit a value.",
...
any [ 'get', 'post' ] => '/settings/department_add/' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        if (body_parameters->get('submit'))
        {
            ...
            if (process(sub { $department->insert_or_update }))
            {
                return forwardHome(
                    {
                        success =>
                        "The $department_name has been created successfully"
                    },
                    'settings/department_overview/'
                );
            }
        ...
        "In this window you can add a $department_name to assign to users.";
...
any [ 'get', 'post' ] => '/settings/department_edit/:id' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        my $id              = route_parameters->get('id');
        ...
        if (body_parameters->get('submit'))
        {
            ...
            if (process(sub { $department->insert_or_update }))
            {
                return forwardHome(
                    {
                        success =>
                        "The $department_name has been updated successfully"
                    },
                    'settings/department_overview/'
                );
            }
        ...
        $department->{type} = $department_name;
        $department->{description} =
"In this window you can edit a ${department_name}. Changes will impact all users currently assigned if you delete or edit a value.";
        $department->{back_url}    = "${base_url}settings/department_overview/";
        $department->{field_label} = ucfirst($department_name);
...
any [ 'get', 'post' ]                           => '/settings/team_overview/' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        if (my $delete_id = param('delete'))
        {
            ...
            if (process(sub { $team->delete_team }))
            {
                return forwardHome(
                    {
                        success =>
                        "The $team_name has been deleted successfully"
                    },
                    'settings/team_overview/'
                );
            }
        }
        ...
        template 'layouts/page_overview_name_only' => {
            ...
            page_description =>
"In this window you can list the ${team_name} that you want to assign users to. You can update the existing items or add new ones. Changes in here will impact all users currently assigned if you delete or edit a value.",
...
any [ 'get', 'post' ]                           => '/settings/team_add/' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        if (body_parameters->get('submit'))
        {
            ...
            if (process(sub { $team->insert_or_update }))
            {
                return forwardHome(
                    {
                        success =>
                        "The $team_name has been created successfully"
                    },
                    'settings/team_overview/'
                );
            }
        ...
        $team->{type} = $team_name;
        $team->{description} =
        "In this window you can add a $team_name to assign to users.";
...
any [ 'get', 'post' ]                           => '/settings/team_edit/:id' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        my $id        = route_parameters->get('id');
        ...
        if (body_parameters->get('submit'))
        {
            ...
            if (process(sub { $team->insert_or_update }))
            {
                return forwardHome(
                    {
                        success =>
                        "The $team_name has been updated successfully"
                    },
                    'settings/team_overview/'
                );
            }
        ...
        $team->{type} = $team_name;
        $team->{description} =
"In this window you can edit a ${team_name}. Changes will impact all users currently assigned if you delete or edit a value.";
        $team->{back_url}    = "${base_url}settings/team_overview/";
        $team->{field_label} = ucfirst($team_name);
...
any [ 'get', 'post' ] => '/settings/report_defaults/' => require_role
    'superadmin'      => sub {
        my $site = var 'site';
...
# telling the browser it's utf-8
    ...
    template 'admin/audit' => {
        ...
        filter_user => $audit->filtering->{user}
            && schema->resultset('User')->find($audit->filtering->{user}),
...
any [ 'get', 'post' ]                           => '/user_upload/' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        if (param 'submit')
        {
            ...
            if (
                process sub {
                    $count = rset('User')->upload(
                        ...
                        permissions =>
                        [ body_parameters->get_all('permission') ],
                        current_user => logged_in_user,
                    );
                }
            )
            {
                return forwardHome(
                    { success => "$count users were successfully uploaded" },
                    'user_overview/');
            }
        }
...
any [ 'get', 'post' ]                           => '/user_export/?' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        if (body_parameters->get('submit'))
        {
            ...
            {
                return forwardHome(
                    { success => "The export has been started successfully" },
                    'user_export/');
            }
        }
        ...
        if (body_parameters->get('clear'))
        {
            ...
            {
                return forwardHome(
                    { success => "The export has been deleted successfully" },
                    'user_export/');
            }
        }
...
any [ 'get', 'post' ]                           => '/user_overview/' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        if (param 'sendemail')
        {
            ...
            {
                report({ is_fatal => 0 },
                    ERROR => 'The groups selected contain no users');
            }
        }
...
any [ 'get', 'post' ]                           => '/user/:id' =>
    require_any_role [qw/useradmin superadmin/] => sub {
        ...
        my $editUser =
        rset('User')->active_and_requests->search({ id => $id })->next
...
any [ 'get', 'post' ] => '/file/:id?' => require_login sub {
    ...
    {
        my $mimetype =
            $filecheck->check_file($upload);    # Borks on invalid file type
        my $file;
        if (process(sub {
            $file = rset('Fileval')->create({
        ...
        {
            my $msg = __x "File has been uploaded as ID {id}",
                id => $file->id;
    ...
    my $fileval =
        $id =~ /^[0-9]+$/ && schema->resultset('Fileval')->find($id)
        or error __x "File ID {id} cannot be found", id => $id;

    # Attached to a record value?
    my ($file_rs) = $fileval
        ->files;    # In theory can be more than one, but not in practice (yet)
    ...
    {
        my $layout =
            var('instances')->layout($file_rs->layout->instance_id);
    ...
    elsif (!$fileval->is_independent)
    {
        ...
        error __ "Access to this file is not allowed"
            unless $fileval->edit_user_id
            && $fileval->edit_user_id == logged_in_user->id;
        $file->schema(schema);
    }
    else
    {
    ...
    if (body_parameters->get('delete'))
    {
        ...
        {
            return forwardHome(
                { success => "File has been deleted successsfully" },
                'file/');
        }
    }
...
get '/record_body/:id' => require_login sub {
    ...
    {
        $version_id
            ? $record->find_record_id($version_id)
...
any qr{/(record|history|purge|purgehistory)/([0-9]+)} => require_login sub {
    ...
    try
    {
              $action eq 'history'      ? $record->find_record_id($id)
            : $action eq 'purge'        ? $record->find_deleted_currentid($id)
            : $action eq 'purgehistory' ? $record->find_deleted_recordid($id)
            :                             $record->find_current_id($id);
    };
    ...
    if (defined param('pdf') && !$record->layout->no_download_pdf)
    {
        ...
        return send_file(
            ...
            filename     => "Record-" . $record->current_id . ".pdf"
        );
    }

    if (query_parameters->get('report')
        && !$record->layout->no_download_pdf)
    {
    ...
    {
        app->execute_hook('plugin.linkspace.record_before_template',
            record => $record);
    }
...
any [ 'get', 'post' ] => '/resetpw/:code' => sub {
    ...
    if (my $username = user_password code => param('code'))
    {
        ...
        if (defined param 'execute_reset')
        {
            ...
            my $audit = GADS::Audit->new(schema => schema, user => $user);
            $audit->login_change(
                "Password reset performed for user ID " . $user->id);
            ...
            report { to => 'syslog' },
                INFO => __x
"debug_login set - new password for username \"{username}\", password: \"{password}\"",
                username => $user->username,
...
# Default pw reset landing page to prevent invalidating the one time use pw reset link, if the page is scanned
    ...
    else
    {
        return forwardHome(
            {
                danger =>
qq(The password reset code is not valid. Please request a new one
...
get '/invalidsite' => sub {
    template 'invalidsite' => { page => 'invalidsite' };
};
...
prefix '/:layout_name' => sub {
    ...
    get '/?' => require_login sub {
        ...
        if (my $dashboard_id = query_parameters->get('did'))
        {
            session('persistent')->{dashboard}->{ $layout->instance_id } =
                $dashboard_id;
        }
        ...
        my $params = {
            ...
            breadcrumbs                       => [
                ...
                Crumb(
                    "$base_url" . $layout->identifier . '/data',
                    "Table: " . $layout->name
                ),
                Crumb("", "Dashboard"),
            ],
...
# times and round up any "to" times.
        ...
        my $records = GADS::Records->new(
            ...
            view_limit_extra_id =>
                current_view_limit_extra_id($user, $layout),
        );
    ...
    get '/data_timeline/:time' => require_login sub {
        ...
        my $fromdt =
            DateTime->from_epoch(epoch => int(param('from') / 1000));
    ...
    any [ 'get', 'post' ] => '/data' => require_login sub {
        ...
        if (param 'modal_delete')
        {
            forwardHome(
                {
                    danger =>
                        "You do not have permission to bulk delete records"
                },
            ...
            $params{limit_current_ids} =
                [ body_parameters->get_all('delete_id') ]
                if body_parameters->get_all('delete_id');
            my $records = GADS::Records->new(%params);

            my $count
                ; # Count actual number deleted, not number reported by search result
            ...
            return forwardHome(
                { success => "$count records successfully deleted" },
                $layout->identifier . '/data');
        }
        ...
        if (param('modal_rewind') || param('modal_rewind_reset'))
        {
            ...
            else
            {
                ...
                my $dt = GADS::DateTime::parse_datetime($input)
                    or error __x "Invalid date or time: {datetime}",
                    datetime => $input;
                session rewind => $dt;
            }
        }
        ...
        if (my $extra =
            $layout->user_can('view_limit_extra') && param('extra'))
        {
            session('persistent')->{view_limit_extra}
                ->{ $layout->instance_id } = $extra;
        }
        ...
        if (param('modal_alert') || param('modal_remove'))
        {
            ...
            if (param('modal_remove'))
            {
                $frequency = '';
                $success_message =
                    "The alert has been removed successfully";
            }
            ...
            my $alert_user =
                  session('views_other_user_id')
                ? rset('User')->find(session('views_other_user_id'))
                : $user;
        ...
        if ($new_view_id)
        {
            session('persistent')->{view}->{ $layout->instance_id } =
                $new_view_id;
        ...
        if ($viewtype = param('viewtype'))
        {
            ...
            {
                session('persistent')->{viewtype}->{ $layout->instance_id }
                    = $viewtype;
            }
        }
        else
        ...
        my $params =
            { layout => var('layout'), };    # Variable for the template
        ...
        if ($viewtype eq 'graph')
        {
            ...
            if (my $png = param('png'))
            {
                ...
                $mech->eval_in_page(
"(function(plotData, options_in){do_plot_json(plotData, options_in)})('$json','$options_in');"
                );
        ...
        elsif ($viewtype eq 'calendar')
        {
            ...
            $params->{calendar} =
                session('calendar');    # Remember previous day viewed
        ...
        elsif ($viewtype eq 'timeline')
        {
            ...
            if (param 'modal_timeline')
            {
                ...
                $tl_options->{all_group_values} =
                    param('tl_all_group_values');
            ...
            if (
                ...
                && (   (!$tl_options->{view_id} && !$view)
                    || ($view && $tl_options->{view_id} == $view->id)
                )                              # Using the same view
                && $tl_options->{now} > DateTime->now->subtract(days => 7)
                ->epoch                        # Within sensible window
                )
            {
                $records->from(
                    DateTime->from_epoch(epoch => $tl_options->{from}));
                $records->to(DateTime->from_epoch(
                    epoch => $tl_options->{to}));
            }
            ...
            $params->{search_limit_reached} =
                $records->search_limit_reached;

            if (my $png = param('png'))
            {
                my $png =
                    _page_as_mech('data_timeline', $params)->content_as_png;
                return send_file(\$png, content_type => 'image/png',);
            }
            if (param('modal_pdf') && !$layout->no_download_pdf)
            {
                $tl_options->{pdf_zoom} = param('pdf_zoom');
                my $pdf = _page_as_mech(
                    'data_timeline', $params,
                    pdf  => 1,
                    zoom => $tl_options->{pdf_zoom}
                )->content_as_pdf(
                    paperWidth  => 16.5,
                    paperHeight => 11.7    # A3 landscape
                );
        ...
        {
            my $globe_options =
                session('persistent')->{globe_options}->{ $layout->instance_id }
                ||= {};
            ...
            $params->{search_limit_reached} =
                $globe->records->search_limit_reached;
        ...
        else
        {
            ...
            {
                $params->{curval_layout_id} =
                    query_parameters->get('curval_layout_id');
                $params->{curval_record_id} =
                    query_parameters->get('curval_record_id');
            }
            ...
            if (param('sort') && param('sort') =~ /^([0-9]+)(asc|desc)$/)
            {
                ...
                forwardHome({ danger => "Invalid column ID for sort" },
                    $layout->identifier . '/data')
                    unless $layout->column($sortcol)
                    && $layout->column($sortcol)->user_can('read');
            ...
            if (param 'modal_sendemail')
            {
                forwardHome(
                    {
                        danger =>
"There are no records in this view and therefore nobody to email"
                    },
                    $layout->identifier . '/data'
            ...
            if (defined param('download'))
            {
                forwardHome(
                    {
                        danger =>
                            "There are no records to download in this view"
                    },
                    $layout->identifier . '/data'
                ...
                return delayed
                {
                    ...
                    try
                    {
                        ...
                        response_header 'Content-Disposition' =>
                            "attachment; filename=\"$now$header.csv\"";
                        content_type 'text/csv; charset="utf-8"';
                    ...
                    accept => 'WARNING-'
                        ;    # Don't collect the thousands of trace messages
        ...
        if (app->has_hook('plugin.linkspace.data_before_template'))
        {
            ...
            app->execute_hook('plugin.linkspace.data_before_template',
                %arg);
        }
    ...
    prefix '/report' => sub {
        ...
        any [ 'get', 'post' ] => '/edit:id' => require_login sub {
            ...
            return forwardHome({ danger => 'Report not found' })
                unless $result;
    ...
    any [ 'get', 'post' ] => '/tree:any?/:layout_id/?' => require_login
        sub {
    ...
    any [ 'get', 'post' ] => '/purge/?' => require_login sub {
        ...
        forwardHome(
            {
                danger =>
                    "You do not have permission to manage deleted records"
            },
            ''
        ...
        if (param('purge') || param('restore'))
        {
            my @current_ids = body_parameters->get_all('record')
                ...
                {
                    danger =>
                        "Please select some records before clicking an action"
                },
                $layout->identifier . '/purge'
                );
            my $records = GADS::Records->new(
                ...
                include_children    => 1,
                view_limit_extra_id =>
                    undef,    # Override any value that may be set
            );
            if (param 'purge')
            {
                my $record;
                $record->purge_current while $record = $records->single;
                forwardHome({ success => "Records have now been purged" },
                    $layout->identifier . '/purge');
            }
            if (param 'restore')
            {
                ...
                forwardHome({ success => "Records have now been restored" },
                    $layout->identifier . '/purge');
            }
        }
    ...
    any [ 'get', 'post' ] => '/graph/:id' => require_login sub {
        ...
        if (param 'delete')
        {
            if (process(sub { $graph->delete }))
            {
                return forwardHome(
                    {
                        success => "The graph has been deleted successfully"
                    },
                    $layout->identifier . '/graphs'
                );
            }
        }
        ...
        if (param 'submit')
        {
            ...
            $graph->$_(param $_)
                foreach (
                    qw/title description type set_x_axis x_axis_grouping y_axis
            ...
            if (process(sub { $graph->write }))
            {
                ...
                return forwardHome(
                    { success => "Graph has been $action successfully" },
                    $layout->identifier . '/graphs');
            }
        }
        ...
        $params->{breadcrumbs}     = [
            ...
            Crumb(
                "$base_url" . $layout->identifier . '/graphs',
                "Manage graphs"
            ),
    ...
    get '/metrics/?' => require_login sub {
        ...
        forwardHome(
            { danger => "You do not have permission to manage metrics" },
            '')
            unless $layout->user_can("layout");
    ...
    any [ 'get', 'post' ] => '/metric/:id' => require_login sub {
        ...
        if (param 'delete_metric')
        {
            if (
                process(sub {
                    $metricgroup->delete_metric(param 'metric_id') }))
            {
                return forwardHome(
                    {
                        success =>
        ...
        if (param 'submit')
        {
            ...
            if (process(sub { $metricgroup->write }))
            {
                my $action = param('id') ? 'updated' : 'created';
                return forwardHome(
                    { success => "Metric has been $action successfully" },
                    $layout->identifier . '/metrics');
            }
        }
        ...
        $params->{header_type} = 'table_title';
        $params->{header_back_url} =
            "${base_url}${tableIdentifier}/metrics";
        $params->{layout_obj}  = $layout;
        $params->{breadcrumbs} = [
            Crumb($base_url . "table/", "Tables"),
            Crumb(
            ...
            Crumb(
                "$base_url" . $layout->identifier . '/metrics', "Metrics"
            ),
    ...
    any [ 'get', 'post' ] => '/topic/:id' => require_login sub {
        ...
        forwardHome(
            { danger => "You do not have permission to manage topics" }, '')
            unless $layout->user_can("layout");
        ...
        {
            $topic =
                schema->resultset('Topic')->new({ instance_id => $instance_id })
                if !$id;
            ...
            $topic->prevent_edit_topic_id(param('prevent_edit_topic_id')
                    || undef);
            ...
            if (process(sub { $topic->update_or_insert }))
            {
                my $action = param('id') ? 'updated' : 'created';
                return forwardHome(
                    { success => "Topic has been $action successfully" },
                    $layout->identifier . '/topics');
            }
        }
        ...
        if (param 'delete')
        {
            if (process(sub { $topic->delete }))
            {
                return forwardHome(
                    {
                        success => "The topic has been deleted successfully"
                    },
                    $layout->identifier . '/topics'
                );
            }
        ...
        template 'topic' => {
            ...
            topics          => [
                schema->resultset('Topic')
                    ->search({ instance_id => $instance_id })->all
            ],
            breadcrumbs => [
                Crumb($base_url . "table/", "Tables"),
                Crumb(
                ...
                Crumb(
                    "",
                    !$id ? 'Add a topic' : 'Edit topic: ' . $topic->name
                ),
            ],
        };
    };
    ...
    any [ 'get', 'post' ] => '/view/:id' => require_login sub {
        ...
        return forwardHome(
            { danger => 'You do not have permission to edit views' },
            $layout->identifier . '/data')
        ...
        if (param 'update')
        {
            ...
            $view->set_groups([ body_parameters->get_all('groupfield') ],);
            ...
            if (process(sub { $view->write }))
            {
              ...
              # And remove any custom sorting, so that sort of view takes effect
                session 'sort' => undef;
                return forwardHome(
                    { success => "The view has been updated successfully" },
                    $layout->identifier . '/data?table_clear_state=1'
                );
            }
        }
        ...
        if (param 'delete')
        {
            ...
            {
                return forwardHome(
                    { success => "The view has been deleted successfully" },
                    $layout->identifier . '/data?table_clear_state=1'
                );
            }
        }
        ...
        my $output = template 'view' => {
            ...
            breadcrumbs     => [
                ...
                Crumb(
                    "",
                    param('clone') ? "Clone view"
                    : $view_id     ? "Edit view: " . $view->name
                    :                "Add view"
                ),
            ],
        };
    ...
    any [ 'get', 'post' ] => '/edit' => require_login sub {
        ...
        forwardHome(
            { danger => "You do not have permission to manage fields" }, '')
            unless $layout->user_can("layout");
        ...
        if (param 'submit')
        {
            ...
            $layout->set_alert_columns(
                [ body_parameters->get_all('alert_column') ]);
            ...
            if (process(sub { $layout->write }))
            {
                ...
                return forwardHome({ success => $msg },
                    $layout->identifier . '/edit');
            }
        }
        ...
        if (param 'delete')
        {
            if (process(sub { $layout->delete }))
            {
                return forwardHome(
                    {
                        success => "The table has been deleted successfully"
                    },
                    'table'
                );
            }
    ...
    any [ 'get', 'post' ] => '/permissions' => require_login sub {
        ...
        {
            $layout->set_groups([
                body_parameters->get_all('permissions') ]);
            ...
            if (process(sub { $layout->write }))
            {
                return forwardHome(
                    {
                        success =>
'The table permissions have been updated successfully'
                    },
                    $layout->identifier . '/permissions'
                );
            }
        }
    ...
    any [ 'get', 'post' ] => '/layout/?:id?' => require_login sub {
        ...
        if (defined param('id'))
        {
            ...
            $params->{instances_object} = var('instances')
                ;    # For autocur. Don't conflict with other instances var
        }
        ...
        if (param('id') || param('submit') || param('update_perms'))
        {
            if (my $id = param('id'))
            {
                ...
                $column->instance_id == $layout->instance_id
                    or error __x
"Column belongs to instance ID {id1} but currently editing instance ID {id2}",
                    id1 => $column->instance_id,
                    id2 => $layout->instance_id;
            }
            else
            {
                my $class = param('type');
            ...
            if (my $delete_id = param 'delete')
            {
                ...
                trace __x "Starting deletion of column {name}",
                    name => $colname;
                my $audit =
                    GADS::Audit->new(schema => schema, user => $user);
                my $username = $user->username;
                my $description =
                    qq(User "$username" deleted field "$colname");
            ...
            if (param 'submit')
            {
                ...
                my @permission_params =
                    grep { /^permission_(?:.*?)_\d+$/ } keys %{ params() };
                ...
                $column->$_(param $_)
                    foreach (
                    qw/name name_short description helptext optional isunique set_can_child
                    multivalue remember link_parent_id topic_id width aggregate group_display/
                    );
                $column->type(param 'type')
                    unless param('id')
                    ; # Can't change type as it would require DBIC resultsets to be removed and re-added
                $column->$_(param $_) foreach @{ $column->option_names };
                ...
                elsif ($column->type eq "enum")
                {
                    ...
                    $column->enumvals({
                        ...
                        enumval_ids =>
                            [ body_parameters->get_all('enumval_id') ],
                    });
                ...
                {
                    $column->refers_to_instance_id(
                        param 'refers_to_instance_id');
                    $column->filter->as_json(param 'filter');
                    my @curval_field_ids =
                        body_parameters->get_all('curval_field_ids');
                    $column->curval_field_ids([@curval_field_ids]);
                }
                elsif ($column->type eq "autocur")
                {
                    my @curval_field_ids =
                        body_parameters->get_all('autocur_field_ids');
                    $column->curval_field_ids([@curval_field_ids]);
                ...
                {
                    my @curval_field_ids =
                        body_parameters->get_all('filval_field_ids');
                    $column->curval_field_ids([@curval_field_ids]);
                    $column->related_field_id(
                        param 'filval_related_field_id');
                }
        ...
        my $back_url =
            defined param('id')
            ? "${base_url}${tableIdentifier}/layout"
            : "${base_url}table";
        my $breadCrumbs = [];
        my $header_type = '';
        ...
        $params->{permission_mapping} =
            GADS::Type::Permissions->permission_mapping;
        $params->{permission_inputs} =
            GADS::Type::Permissions->permission_inputs;
        $params->{topics} = [ schema->resultset('Topic')
                ->search({ instance_id => $layout->instance_id })->all ];
        $params->{content_block_custom_classes} = 'content-block--footer';
        ...
        if (param 'saveposition')
        {
            ...
            if (process(sub { $layout->position(@position) }))
            {
                return forwardHome(
                    {
                        success =>
                            "The ordering has been saved successfully"
                    },
                    $layout->identifier . '/layout'
                );
            }
    ...
    any [ 'get', 'post' ] => '/approval/?:id?' => require_login sub {
        ...
        if (param 'submit')
        {
                ...
                )
            {
                ...
                if ($col->userinput
                    && defined $newv)    # Not calculated fields
                {
                    $failed = !process(
                        sub {
            ...
            if (!$failed && process(sub { $record->write }))
            {
                return forwardHome(
                    ...
                    $layout->identifier . '/approval'
                );
            }
        }

        my $page;
        my $params = { page => 'approval', };
        ...
        if ($id)
        {
            ...
            $params->{record_presentation} = $record->presentation(
                edit     => 1,
                new      => $approval_of_new,
                approval => 1
            );
        ...
        if (param 'submit')
        {
            ...
            else
            {
                ...
                $result = process(sub { $record->write })
                    && process(
                        sub { $record->write_linked_id(param 'linked_id') });
            }
            if ($result)
            {
                return forwardHome(
                    { success => 'Record has been linked successfully' },
                    $layout->identifier . '/data');
            }
        }
    ...
    post '/edits' => require_login sub {
        ...
        while (my ($id, $values) = each %$records)
        {
            ...
            $layout = $record
                ->layout;    # May have changed if record from other datasheet
            ...
            else
            {
             ...
             # Therefore subtract one day from it
                unless (
                    process sub {
                        $record->fields->{ $values->{column} }
                            ->set_value($to_write, subtract_days_end => 1);
                    }
                    )
                {
                    $failed = 1;
        ...
        {
            return forwardHome(
                { success => 'Submission has been completed successfully' },
                $layout->identifier . '/data'
            );
        }
    };
    ...
    any [ 'get', 'post' ] => '/bulk/:type/?' => require_login sub {
        ...
        my %params = (
            ...
            ,    # Need all columns to be able to write updated records
            schema              => schema,
            user                => $user,
            layout              => $layout,
            view_limit_extra_id =>
        ...
        if (param 'submit')
        {
            ...
            foreach my $col ($record->edit_columns(new => 1, bulk => $type))
            {
                ...
                my $included = body_parameters->get('bulk_inc_' . $col->id)
                    ;    # Is it ticked to be included?
                report
                    WARNING => __x
"Field \"{name}\" contained a submitted value but was not checked to be included",
                    name => $col->name
                    if join('', @newv) && !$included;
                next
                    unless body_parameters->get('bulk_inc_' . $col->id)
                    ;    # Is it ticked to be included?
                my $datum = $record->fields->{ $col->id };
                my $success =
                    process(sub { $datum->set_value(\@newv, bulk => 1) });
                push @updated, $col
                    if $success;
            ...
            if (!$failed_initial)
            {
                ...
                while (my $record_update = $records->single)
                {
                    ...
                    {
                        my $newv =
                            [ body_parameters->get_all($col->field) ];
                        last if $failed = !process(
                            sub {
                                $record_update->fields->{ $col->id }
                                    ->set_value($newv, bulk => 1);
                            }
                        );
                    }
                    $record_update->fields->{ $_->id }
                        ->re_evaluate(force => 1)
           ...
           # records will actually be made blank, as we wouldn't write otherwise
                        if (
                            process(sub {
                                $record_update->write(force_mandatory => 1);
                            })
                            )
                        {
                            $success++;
                ...
                {
                    my $msg =
                        __xn "{_count} record was {type}d successfully",
                        "{_count} records were {type}d successfully",
                        $success, type => $type;
                    return forwardHome(
                ...
                {
                    my $s = __xn "{_count} record was {type}d successfully",
                        "{_count} records were {type}d successfully",
                        ($success || 0), type => $type;
                    my $f = __xn ", {_count} record failed to be {type}d",
                        ", {_count} records failed to be {type}d",
        ...
        my $count_msg = __xn ", which contains 1 record.",
            ", which contains {_count} records.", $count;
        if ($type eq 'update')
        {
            my $notice = session('search')
                ? __x(
                    qq(Use this page to update all records in the
                    ...
                    The current search is "{search}"),
                    search => session('search')
                )
                : $params{limit_current_ids} ? __x(
                    qq(Use this page to update all currently selected records.
                    Tick the fields whose values should be updated. Fields that are
                    not ticked will retain their existing value.
                    The current number of selected records is {count}.),
                    count => scalar @{ $params{limit_current_ids} }
                )
                : __x(
                    qq(Use this page to update all records in the
        ...
        else
        {
            my $notice = session('search')
                ? __x(
                    qq(Use this page to bulk clone all of the records in
                    ...
                    with their current value. The current search is "{search}"),
                    search => session('search')
                )
                : $params{limit_current_ids} ? __x(
qq(Use this page to bulk clone all currently selected records.
                ...
                : __x(
                    ...
                    the same existing values by default, but replaced with the values below
                    where that value is ticked. Values that are not ticked will be cloned
                    with their current value. The current view is "{view}"),
                    view => $view_name
                );
            my $msg = $notice;
            $msg .= $count_msg unless $params{limit_current_ids};
        ...
        template 'edit' => {
            view   => $view,
            record =>
                $record->presentation(edit => 1, new => 1, bulk => $type),
            bulk_type                    => $type,
            page                         => 'bulk',
            header_type                  => "table_title",
            content_block_custom_classes => 'content-block--footer',
    ...
    any [ 'get', 'post' ] => '/import/?' => require_login sub {
        ...
        forwardHome(
            { danger => "You do not have permission to import data" }, '')
    ...
    get '/import/rows/:import_id' => require_login sub {
        ...
        template 'import/rows' => {
            ...
            breadcrumbs     => [
                ...
                Crumb(
                    "$base_url" . $layout->identifier . '/import',
                    "Import records"
                ),
    ...
    any [ 'get', 'post' ] => '/import/data/?' => require_login sub {
        ...
        if (param 'submit')
        {
            ...
            {
                my %options =
                    map { $_ => 1 } body_parameters->get_all('import_options');
                $options{no_change_unless_blank} = 'skip_new'
                    if $options{no_change_unless_blank};
                $options{update_unique} = param('update_unique')
                    if param('update_unique');
                $options{skip_existing_unique} =
                    param('skip_existing_unique')
                    if param('skip_existing_unique');
                ...
                if (process sub { $import->process })
                {
                    return forwardHome(
                        {
                            success =>
"The file import process has been started and can be monitored using the Import Status below"
                        },
                        $layout->identifier . '/import'
                    );
                }
            }
            else
    ...
    any [ 'get', 'post' ] => '/graphs/?' => require_login sub {
        ...
        if (param 'graphsubmit')
        {
            if (
                process(sub {
                    $user->graphs($layout->instance_id,
                        [ body_parameters->get_all('graphs') ]);
                })
                )
            {
                return forwardHome(
                    { success => "The selected graphs have been updated" },
                    $layout->identifier . '/data'
                );
            }
        }
    ...
    get '/match/user/' => require_login sub {
        ...
        my $layout = var('layout') or pass;
        $layout->user_can("layout")
            or error "No access to search for users";
...
# I don't know where else this is used, so I am going to revert it to it's original setup and leave it alone for now!
    get '/match/layout/:layout_id' => require_login sub {
        ...
        return to_json {
            error   => 0,
            records => [
                $column->values_beginning_with(
                    $query,
                    noempty => query_parameters->get('noempty'),
                    use_id  => query_parameters->get('use_id')
                )
            ],
        };
    };
...
{   my ($id, $record) = @_;
    ...
    $params{layout} = var('layout')
        if var('layout');    # Used when creating a new record
        ...
        || defined(param 'validate'))
    {
        ...
        if (defined(param 'validate'))
        {
            ...
            my $source_curval =
                $layout->column(param('source'), permission => 'read');
            try {
                $record->write(dry_run => 1, parent_curval => $source_curval)
            };
            if (my $e = $@->wasFatal)
            {
                push @validation_errors,
            ...
            my $return = encode_json({
                ...
                values => +{
                    map {
                        $_->field => $record->fields->{ $_->id }->as_string
                    } @{ $source_curval->curval_fields }
                },
            });
        ...
        elsif (param 'draft')
        {
            if (
                process sub {
                    $record->write(
                        draft            => 1,
                        submission_token => param('submission_token')
                    );
                }
                )
            {
                return forwardHome(
                    { success => 'Draft has been saved successfully' },
                    $layout->identifier . '/data');
            }
        ...
        elsif (!$failed)
        {
            ...
            {
                my $forward =
                    (!$id && $layout->forward_record_after_create)
                    || param('submit') eq 'submit-and-remain'
                    ? 'record/' . $record->current_id
                    : $layout->identifier . '/data';
                return forwardHome(
                    {
                        success =>
'Submission has been completed successfully for record ID '
                            . $record->current_id
                    },
                    $forward
                );
    ...
    my $child_rec =
          $child && $layout->user_can('create_child') ? int(param 'child')
        : $record->parent_id                          ? $record->parent_id
        :                                               undef;

    notice __
        "Values entered on this page will have their own value in the child "
    ...
    my $recordPresentation = $record->presentation(
        edit  => 1,
        new   => !$id,
        child => $child,
        modal => $modal
    ...
    my $params = {
        ...
        breadcrumbs           => [
            ...
            Crumb(
                "",
                $id
                ? ucfirst($layout->record_name) . " ID: "
                    . $recordPresentation->{current_id}
                : "New " . $layout->record_name
            ),
        ],
    };
    ...
    if ($id)
    {
        $params->{content_block_custom_classes} =
            'content-block--record content-block--footer';
    }
    else
    {
        $params->{content_block_custom_classes} = 'content-block--footer';
    ...
    $params->{modal_field_ids} =
        encode_json $layout->column($modal)->curval_field_ids
        if $modal;
